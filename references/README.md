...
| `text-rendering-tests` | **The shaping conformance suite.** Its expectations are written by hand from the OpenType specification, which is what makes it an oracle for shaping rather than a recording of it — Vixen delegates shaping to HarfBuzz, so a HarfBuzz-versus-HarfBuzz comparison would prove nothing. Unlike everything else in this table, its fonts *are* redistributed: an expectation is only meaningful against the exact font it was written for. See `Core/Vixen.Ui.Text.Tests/Fonts/README.md`. | doc 09, doc 12 |
