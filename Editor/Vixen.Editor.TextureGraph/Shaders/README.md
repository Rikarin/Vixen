...
⚠ **Two agents of one batch corrected it independently, to two different numbers**, because one of
them was adding `Mix` in the same hour — which is the warning below arriving as a merge conflict.
The table is kept for the *derivation*; the count itself is `TextureKernels.Names.Count` and is never
worth reading off a file.
