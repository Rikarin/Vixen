...
public sealed class TerrainModeTests {
    ...
    public void Entering_the_mode_claims_the_digits_and_leaving_it_gives_them_back() {
        ...
        Assert.Equal(
            TerrainMode.ToolCommand(TerrainTool.Flatten),
            shell.Keys.CommandFor(chord, TerrainMode.TerrainContext)
        );
    }

    [Fact]
    public void Every_tool_has_its_own_digit_and_none_of_them_collide() {
        var (shell, _) = Built();
        using var _shell = shell;

        var chords = TerrainMode.Tools
            .Select(tool => shell.Keys.ChordFor(TerrainMode.ToolCommand(tool)))
            .ToArray();

        Assert.All(chords, chord => Assert.NotEqual(KeyChord.None, chord));
        Assert.Equal(chords.Length, chords.Distinct().Count());

        // 1…8, in the order the strip lists them, which is what makes the digit a person can guess.
    ...
    [Fact]
    public void The_mode_bar_carries_the_eight_tools_only_while_terrain_is_active() {
        var (shell, _) = Built();
        using var _shell = shell;
    ...
    public void Unregistering_the_mode_takes_its_commands_with_it() {
        ...
        foreach (var id in TerrainMode.BrushCommands.Concat(TerrainMode.LayerCommands)) {
