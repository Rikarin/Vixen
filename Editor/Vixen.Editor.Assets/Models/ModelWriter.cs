...
public static class ModelWriter {
    ...
    ///     single most common way a hand-written OBJ exporter is wrong — and it is wrong in a way that
    ///     produces a file that opens, with the second object's faces made of the first object's
    ///     vertices.
    /// </remarks>
        ...
        foreach (var mesh in meshes) {
            ...
            for (var index = 0; index + 2 < mesh.Indices.Length; index += 3) {
                ...
                for (var corner = 0; corner < 3; corner++) {
                    var vertex = offset + mesh.Indices[index + corner];

                    text.Append(' ').Append(Number(vertex));
