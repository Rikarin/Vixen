...
writing a signal *only ever queues*, and `Flush` drains it after input and before layout, because an
effect running at the write would mutate the tree while the renderer walked it (ADR-007). (It drains
again after each `LayoutFinished`, before the draw, so a signal a layout handler writes — a pooled
row's index — is drawn on its own frame; see #1406. Neither drain reaches a caller between frames.)
⚠ **This one is invisible from the panel's own source — only from its callers.**
...
severity colours, which `AddressableGroupsView` passed positionally until #89 ported it, but `class` is one of markup's
...
⚠ **This paragraph said "three test files in the whole editor dump a tree", then "eight", and both
...
"a committed dump rather than a wave note". There are **fifteen** (`ConsoleViewDumpTests`, #758,
and `BehaviorSearchPopupDumpTests`, #89, landed on the same day as the twelfth and thirteenth;
`AssetGridDumpTests`, #1406, is the fourteenth, and `AddressableGroupsViewDumpTests`, #89, the fifteenth).
...
`UiTest.Tree` or `UiTest.Flags` and requires the set to be exactly the rows below — so a tenth dump
file cannot be written without this table growing, and a row cannot outlive the file it names. The
prose is still prose; the list is a measurement.
...
| `Vixen.Editor.App.Tests/AssetGridDumpTests` | `AssetGrid`, the Project panel's tiles, in five states reached through the interface, recorded from the C# `CreateTile`/`BindTile` control *before* its `@rows` port (#1406) |
...
facts together as progress is the mistake this section exists to prevent.
...
⚠ **This table is gated against [`docs/MarkupPending.txt`](../../docs/MarkupPending.txt)
([#1342](https://github.com/Rikarin/Vixen/issues/1342)).** The ledger file is the count and this is the
reasoning, and for four waves they disagreed with nothing red — `SceneHierarchyView` declined here
while listed pending there. `MarkupRatioTests.The_readme_port_table_agrees_with_the_ledger_about_every_view_it_names`
reads each row's **live** verdict (the cell with its `~~struck~~` history removed): a view the ledger
calls `pending` needs an open row, a `canvas`/`machinery`/`not-a-view` one needs a row starting
**no** or **not a panel**, and a view with a `.vxml` may be neither declined nor left open. So a
port or a decline owes both files, and a row's current verdict goes **first** in its cell. ⚠ Keep
every row on one line — a blank line inside a cell ends the table, which is how every row below
`ComponentsView` rendered as loose pipe text until the same issue; use `<br><br>` instead.
...
| `ComponentsView` | snapshot | no | ~~chrome only — the foldout bodies are `IPropertyDrawer` output.~~ **done, wave 10 (2026-09-04)** — the history below is oldest first, and the port is its last paragraph. ⚠ Until #1342 this cell was two paragraphs with a blank line between them, which in Markdown ends a table: every row after it rendered as loose pipe text and no reader saw the declines as a table. ⚠ **Wave 8 reached it and stopped, and the sizing is what it got wrong.** "Chrome only" is three elements here — `component-list`, `component-drop-indicator` and the Add button — and the work is the *foldout loop* the row does not mention: one `Expander` per bridge, each with a `Document.Move(glyph, 1)` putting its icon between the chevron the header made and the header's own label, a routed `DragEvent` handler per header, and a `Sections` projection that `Drop` scans with `ReferenceEquals` while `Aim` does bounds arithmetic against `<components>` as the positioned ancestor. Four tests drive it with a real pointer drag. That is an M for the loop and an S for the chrome, and taking the S alone would move nothing. ⚠ **Declined a third time, wave 9 (2026-08-24) — and wave 8's four obstacles are really one, which is worth more than the decline.** Three of the four are not about the drag at all. `Sections` is `host.Children.OfType<Expander>()`, so a keyed `@for` producing one `<Expander>` per bridge yields *exactly* that projection and `Drop`'s `ReferenceEquals` scan survives untouched; `Aim` measures `Bounds` against this control's `AbsoluteLeft`/`AbsoluteTop`, which markup does not change either. Those three would port for free. **The one that blocks it is `Expander.ContentHost => Content`.** Nested tags under a control go to its content host, so markup can fill a foldout's *body* and has no spelling for its **header** — and the header is where all three of this panel's per-section additions go: the icon (`Document.Move(glyph, 1)`, deliberately between the chevron the control made and the label it made), the remove button, and the routed `DragEvent` that is the grab handle. So the `@for` would produce three empty foldouts and C# would still build every header, which is the whole of the loop. <br><br> ⚠ **The escape exists and is why this is a decline rather than a "no": `use="@(fold => Header(fold, bridge))"` would run the header surgery per row**, since `use` is the sanctioned spelling for a control fed by a method. It is refused here on taste rather than on capability — `use` is an *effect*, so a header-building expression is one signal read away from appending the icon twice, and the four tests behind it drive a real pointer drag. ~~**The honest fix is `Expander` publishing a header slot**, at which point the icon needs no `Move` and the port is ordinary. Whoever takes this should build that first and port second~~ **Done, wave 10 (2026-09-04), and in that order.** `slot="header"` is a language feature now — a child of a capitalised tag may name one of the parent's published slots, answered by `UiElement.NamedHost` on a control and by the `<slot name="…" />` dictionary on a component — and `Expander` publishes `header`. Wave 9's read of the four obstacles was exactly right: `Sections`, `Aim` and `Drop` moved unchanged. 901 lines of C# → a 777-line `.vxml` and a 271-line `.cs` of `Default`, `Registered` and `CategoryOf`, none of which reads an element (⚠ the wave note first said 861 → 707 + 208; the three counts are `wc -l` on the files as committed, which is the number a reader can check). **Three things the sizing did not see, and two of them cost an hour each.** ⚠ The icon needs no `Move` and it needs *two lines of CSS*: a slot appends, so `order: 1` on the label and `order: 2` on the remove button is what puts the glyph between the chevron and the name — `LayoutStyle.Order` has been implemented all along and no editor sheet was using it. ⚠ **`on:drag` is the middle stage only.** `AddHandler<DragEvent>` sees all four; the runtime's `on:` table splits them across `dragstart`/`drag`/`dragend`, so the one-attribute version compiled, bound, fired on every pointer move and never started or finished a drag — no exception, nothing reordered. ⚠ And the new `<component-fields>` wrapper hit the CSS-initial-`row` trap `component-list` above it already documents: every property of a component laid out *across* the panel in one 30-pixel strip. ~~**The port's own limitation, named rather than glossed: markup can fill a part and still cannot subscribe to one.**~~ **Closed, and the workaround with it (#586).** `on:` is an attribute on a tag and `Expander.Header` is a part, so for one wave the drag was subscribed on the foldout and an eleven-line `Grabbed` walked up from `DragEvent.Source` to reproduce what bubbling to the header selected. `on:dragstart.slot-header` is the spelling now — it emits `ctx.On(BuildContext.Into(n1, "header"), …)`, which is the `fold.Header.AddHandler<DragEvent>(…)` the hand-written panel made and not a filter that agrees with it — and the handler knows its own row, so `Rearrange` derives nothing from the event and `Grabbed` is gone. ⚠ **A handler on a part keeps the whole gesture**, because `Gestures` raises every stage from `Press.Target`, the element the pointer went *down* on: the route is fixed for the drag, so a header handler still receives the drop wherever the pointer ends up. Evidence is `ComponentsViewDumpTests` — the header tree in full, a flags dump in four states, every state reached through the interface (a click, a click, a real pointer drag), the `order` rules sabotage-tested red, and two facts added for the gesture itself: a drag begun in a foldout's *body* reorders nothing (red when the subscription goes back to the whole foldout, which is the regression a header-only test cannot see), and the drop line is read mid-drag and compared with where the release actually put the foldout | M |
| `MoveSetView` · `ProxyShapeView` | live | no | ~~**defer.**~~ **pending** — the half that was unportable was the field rows, which `change:` now expresses. Both are forms over a document with no `OnDraw` | ~~L–XL~~ M |
| `SequenceView` · `BehaviorTreeView` · `SpriteSheetView` · `AnimationGraphView` | live | no | ~~**defer.**~~ ~~The half that was unportable was the field rows, which `change:` now expresses~~ **no — canvases** (`docs/MarkupPending.txt`, 2026-09-22): a timeline, a node canvas, slices hit-tested in image pixels, and `StateMapView`'s `OnDraw`. ⚠ This row used to group them with the two forms above as M–L ports, and the ledger's line-by-line read says the field rows were never the whole of them — the element that remains after the fields move is one element with pan, zoom and hit-testing inside it. `AnimationGraphView` still has no tests at all | — |
...
| `ConsoleView` | live | yes, additively — the model's visible set, as a generation | **done (#758, 2026-09-24).** ~~**no** — `VirtualizingPanel` row template~~ ~~**pending #758**~~. `ConsoleView.vxml` is the toolbar, an `@rows` block over the `VirtualizingPanel` and the detail pane; the five columns are bindings through the slot's `index`, the row's `on:click` is the tap (and its `Count` the double click that raises `Activated`), `:checked` is a `use=` effect, and the pane is an `@if` over a selection compared **by reference** — `LogRecord` is a record, so two identical lines in the same millisecond would otherwise be one selection. ⚠ **The model decision**: `ConsoleModel`'s visible set is a buffer rewritten in place, which is how it avoids the per-line allocation, so what it can offer a signal is a *generation* — its `Changed` in signal form, read by every public reading of the set and the badges. A snapshot would be ten thousand indices per logged frame, and the view keeps no revision of its own. `ConsoleViewDumpTests` was **committed before the port** from the hand-written control (seven states, tree and flags); the port matches three byte for byte and four differ only in `.parked` 0×0 rows, which the C# left holding stale text and, after Clear, a stale `Checked` bit — the message log's finding again. Five software captures are byte-identical PNGs and two differ only in the digits of `thread N` | M |
| `AssetGrid` | live | yes, additively — a `version` for what a slot shows and a `marks` for `:checked` | ~~**pending, and no longer #758's**~~ **done ([#1406](https://github.com/Rikarin/Vixen/issues/1406), 2026-09-24).** `AssetGrid.vxml` is the breadcrumb bar and an `@rows` block over the `VirtualizingGrid` — the last editor panel's virtualised template that was set by hand (`CreateTile`/`BindTile`; `TreeView` still sets its own, as a control), and the first production caller of the grid half of `RowPool`. ⚠ **A tile is an element now, not a type**: a pool makes every slot by tag name and a capitalised tag cannot be the row (VXML2027), so the row is `<asset-tile>` writing the `variant-default size-md` a control used to write for itself, `use=` sets the focusability `Control.OnCreated` used to, and `AssetTile` is a view over a realised slot — `Tiles`, `TileOf` and `TileAt` hand it out, and the tests that laid a pointer on a tile reach its element through `AssetTile.Element`. ⚠ **The model decision**: a slot is rebound by writing its index, and entering a folder rewrites every slot with the index it already had, as does a thumbnail arriving for a tile already drawn — so every column also reads a `version` that `Show` and `Refresh` move; sabotaged to `Peek()` it reddens six tests. `AssetGridDumpTests` was **committed before the port** from the C# control (five states, tree and flags) and the port matches all ten byte for byte; the Project panel's region of fifteen software and Vulkan captures is byte-identical, the rest of the frame differing only in the inspector's GUID, which is a fresh project each run. ⚠ **Settled pictures could not see its one regression**: the grid realises from `LayoutFinished`, after the frame's only effect flush, so the first port drew every scrolled tile showing its previous item for one frame and a press in it chose that item — fixed by draining effects in `UiDocument.Settle` and by `TileAt` asking the grid's window rather than a map a binding wrote, and pinned by tests that scroll and run exactly one frame. ⚠ The ledger still does not list it (`Vixen.Editor.App/AssetGrid.vxml`), because it counts only classes named `*View`, `*Panel`, `*Inspector` or `*Popup` | M |
...
| `ProfilerView` · `LayerStackView` · `PaintBrushInspector` | mixed | no | **pending** — never surveyed in this table; each one's model decision is named on its line in `docs/MarkupPending.txt`. `ProfilerView`'s candidate is the per-scope table beside the flame chart, not the chart | — |
| `AddressableGroupsView` | snapshot | the analysis rows, yes; the group list, deliberately no | ~~**pending**~~ **done (#89, 2026-09-24).** ⚠ The ledger's model line was half right: the `List<AssetEntry> groups` it warned about feeds a `TreeView`, which is filled by `Root.Add` calls in every panel that has one, so it stays the snapshot `Rebuild` takes and is not made a signal nothing would read. What was a list of elements is the analysis, now a keyed `@for` of `AnalysisRow`s over one `Signal<ImmutableArray<Note>>` written whole by `Run`; each note carries its index so two identical diagnostics stay two rows, and `AnalysisRows` counts the model because the elements follow one flush later. `AddressableGroupsViewDumpTests` was committed before the port, from the hand-written control, in five states (two reached by clicking a row and Analyse build, one by a second run with a shorter plan); the port matches all ten dumps and all five software captures byte for byte. Sabotage: the rows read through `Peek()` reddens the three states with rows; the policy's `hidden` class dropped reddens four. ⚠ Recording found the summary line and the no-planner sentence painted in `--warning` since the panel was written; the port keeps it, and it is its own fix | S |
| `BehaviorSearchPopup` | snapshot | the ranked list, yes | ~~**pending**~~ **done (#89, 2026-09-24), after #1370 made it reachable at all.** ⚠ Nothing opened this popup until the same batch wired Space, Add node and the two attachment buttons to it, so a port before that would have been a port of a panel no person could see; and recording its reference then found that it could not show what it offered — fourteen composite-and-task rows in a 320 px box with `overflow: hidden` clipped the last two out of every pointer's reach and squeezed the field from 34 px to 27. Both were fixed in the C# (a `ScrollView` list, a field that does not shrink, a clamp to the window) **before** the reference was taken. The model decision is one `Signal<ImmutableArray<BehaviorNodeType>>` written by `Rank`, and the rows are a keyed `@for` over the types where the C# removed and rebuilt every row on every keystroke; a row is still picked by its position, since the `@for` writes rows in `found`'s order. `BehaviorSearchPopupDumpTests` was committed before the port, from the hand-written control, in five states reached through the tree view's gestures; the port matches every tree and flags dump byte for byte and the five software-rasterised captures are byte-identical PNGs. Sabotage: the rows read through `Peek()` reddens six tests; the field's class dropped from the markup reddens six. ⚠ **No dump can see the keyed `@for`'s claim**, since a rebuilt row prints the same as a kept one: `A_row_still_offered_after_a_keystroke_is_the_same_element_where_it_moved_to` holds it by identity (Random selector moves from row 3 to row 1 on "sel" and is the same element), red against the hand-written control and red with the key a fresh object while every dump stays green | S |
| `VfxPreviewView` · `PaintMeshView` · `PaintUvView` · `TextureGraphView` | live | no | **no — canvases**: `VfxPreviewView` projects every particle through a fixed orbit camera in `OnDraw`, the two paint views paint through a raster, and `TextureGraphView` is a node canvas | — |
...
See the panel ledger's `MessageLogView` row. ~~`ConsoleView` stays — its detail pane is the same
shape, but its rows carry five columns and a double-tap that opens a source file, and it has no
reference dump yet to be held to.~~ ✅ **`ConsoleView` is ported too (#758, 2026-09-24)**: the
reference dump was committed first, from the hand-written control, and the double tap is the row's
`on:click` reading `TapEvent.Count`. See its row in the ledger.
