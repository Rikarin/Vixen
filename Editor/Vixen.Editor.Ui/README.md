...
| `AssetGrid` | live | no | **pending, and no longer [#758](https://github.com/Rikarin/Vixen/issues/758)'s** — #758 built `@rows` and was finished by the `ConsoleView` port, the only work its criterion named; this row is its own follow-up. It still fills its `VirtualizingGrid` from C# (`CreateTile`/`BindTile`, `AssetGrid.cs:142-145`), the grid half of `RowPool` that has no production caller; `@rows` fills a grid as it fills a panel, and `ConsoleView` and `MessageLogView` are the two ports over it. ⚠ The ledger does not list it (`Vixen.Editor.App/AssetGrid.cs`), because it counts only classes named `*View`, `*Panel`, `*Inspector` or `*Popup`, so the gate cannot see this row's verdict | — |
