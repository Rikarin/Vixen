...
and `BehaviorSearchPopupDumpTests`, #89, landed on the same day as the twelfth and thirteenth; `AssetGridDumpTests`, #1406, is the fourteenth).
...
| `AddressableGroupsView` · `ProfilerView` · `LayerStackView` · `PaintBrushInspector` | mixed | no | **pending** — never surveyed in this table; each one's model decision is named on its line in `docs/MarkupPending.txt`. `ProfilerView`'s candidate is the per-scope table beside the flame chart, not the chart | — |
