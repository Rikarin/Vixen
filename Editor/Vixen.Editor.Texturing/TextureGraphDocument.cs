...
///     </para>
///     <para>
///         ⚠ <b>Nothing here compiles.</b> <c>ShaderGraphDocument</c> has a <c>Compile()</c> and this
///         has not, because <c>TextureGraphCompiler</c> is <c>internal</c> to
///         <c>Vixen.Editor.TextureGraph</c> and this assembly is not on its
///         <c>InternalsVisibleTo</c> list. A method that threw, or one that returned an empty plan,
///         would be a worse answer than the absence: see the README, which names the one-line change
///         that would close it and why this slice did not make it.
///     </para>
///     <para>
...
public sealed class TextureGraphDocument : EditorDocument {
    ...
    public TextureGraphDocument(
        ...
        AssetPath = path;
        Registry = registry ?? TextureNodeLibrary.Create();
