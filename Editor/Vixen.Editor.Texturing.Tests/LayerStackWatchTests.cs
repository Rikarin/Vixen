...
///         number each reader keeps its own copy of — so that one asks nothing at all.
///     </para>
///     <para>
...
///     </para>
///     <para>
///         ⚠ <b>Driven through <c>ExternalEdits.Apply</c>, never by moving the number.</b> The
///         notification is the mechanism: a test that assigned <c>ModelsRevision</c> would be green
///         against a document with no <c>OnProjectFileChanged</c> override at all, which is #954's
///         own warning one issue later — and is why the setter is private.
///     </para>
/// </remarks>
public class LayerStackWatchTests {
    ...
    public void A_meta_sidecar_tells_the_picker_to_re_ask() {
        ...
        // What a consumer keeps, and the reason this is a difference rather than `Assert.NotEqual(0,
        // …)`: the number starts at one so that a picker is filled once before anything has happened.
    ...
    ///         <c>ModelsRevision</c> unconditional is cheap because its reader refills once and
    ///         records it; making the <em>compound</em> flag unconditional would rebuild every node type
