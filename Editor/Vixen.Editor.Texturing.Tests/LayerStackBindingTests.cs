...
public class LayerStackBindingTests {
    ...
    /// <summary>A model re-exported under an open stack reaches the pane.</summary>
    /// <remarks>
    ///     <para>
    ///         <b><a href="https://github.com/Rikarin/Vixen/issues/971">#971</a>, by the route the
    ///         issue does not name and which needs no import at all.</b> The resolve is cached on the
    ///         stack's path, its model and the set's mesh, and not one of the three moves when the
    ///         artist re-exports the model over the top of itself — so the pane goes on drawing the
    ///         previous export's islands and the brush goes on accepting the previous export's texels,
    ///         with nothing anywhere saying the answer is stale.
    ///     </para>
    ///     <para>
    ///         ⚠ <b>The segment count is the instrument, not the field.</b> A test that read the model
    ///         path back would pass against exactly this defect: the binding was never wrong, the
    ///         geometry behind it was. One quad is six segments and two quads are twelve, so the
    ///         re-export is visible as a number rather than as a picture.
    ///     </para>
    /// </remarks>
    [Fact]
    public void A_model_re_exported_under_an_open_stack_reaches_the_pane() {
        using var fixture = new TexturingFixture(graphics: true);
        ...
        Assert.Equal(6, ImageIn(PaintPane(fixture)).Overlay.Count);

        // The artist re-exports, with a second object in the file this time.
        Model(fixture, "Hull.obj", Quad(0f, 0.4f) + Named("upper", 4, 0.6f, 1f));
        Refresh(fixture);

        Assert.Equal(12, ImageIn(PaintPane(fixture)).Overlay.Count);
    }

    /// <summary>An import reaches a stack that was already open when it ran.</summary>
    /// <remarks>
    ///     <para>
    ///         <b><a href="https://github.com/Rikarin/Vixen/issues/971">#971</a>'s own route, and the
    ///         file's own bytes cannot see it.</b> The cube here has no <c>vt</c> line anywhere in it,
    ///         so an atlas can only have been generated by the import's unwrap — the file is byte for
    ///         byte what it was when the stack bound to it, and only <c>Library/</c> changed. A cache
    ///         key made of the file's stamp is therefore not enough; what moves is
    ///         <c>ProjectMeshSource.Revision</c>, which <c>ContentTasks</c> already bumps when an
    ///         import finishes.
    ///     </para>
    ///     <para>
    ///         ⚠ <b>The refusal before the import is what makes the second half mean anything.</b>
    ///         Without it a resolve that read the source file would satisfy the ending, and the source
    ///         file has nothing to read: the first assertion is that the pane is empty, which is the
    ///         state the artist is looking at while the import runs.
    ///     </para>
    /// </remarks>
    [Fact]
    public async Task An_import_reaches_a_stack_that_was_open_while_it_ran() {
        using var fixture = new TexturingFixture(graphics: true);
        var workspace = new ProjectWorkspace(fixture.Paths);
        var geometry = new ProjectMeshSource(workspace);

        fixture.Services.Add<IMeshSource>(geometry);
        Model(fixture, "Hull.obj", Cube);

        var sidecar = AssetMetaFile.PathFor(fixture.Paths.Absolute("Assets/Hull.obj"));

        AssetMetaFile.WriteFile(
            sidecar,
            AssetMetaFile.ReadFile(sidecar) with {
                Importer = new ModelImportSettings { Unwrap = UnwrapMode.Always, UnwrapResolution = 256 }
            }
        );
        ...
        Assert.Empty(ImageIn(PaintPane(fixture)).Overlay);
        ...
        // What the editor does on the frame after an import finishes — see `ContentTasks.Pump`.
        Assert.Equal(0, geometry.Revision);
        geometry.Invalidate();
        Assert.Equal(1, geometry.Revision);

        Refresh(fixture);

        // A cube is twelve triangles and three segments a triangle. The file has no `vt` in it, so
        // every one of them came out of `Library/`.
        Assert.Equal(36, ImageIn(PaintPane(fixture)).Overlay.Count);
    }
