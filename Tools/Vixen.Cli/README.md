...
here. `plugin` is blocked rather than pending: it would have to pin a `PackageReference` on
`Vixen.Editor.Plugin`, which does not exist, and a template producing a project that will not restore
fails at the one moment a person has no context to debug it. `tool` is unblocked and simply not
written. See [`Vixen.Templates`](../Vixen.Templates/README.md) § Still to come.
