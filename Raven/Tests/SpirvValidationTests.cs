...
public class SpirvValidationTests {
    ...
    [InlineData("a while loop", """
        ...
        """)]
    [InlineData("a loop that returns out of its middle", """
                for (i in 0 .. 4) {
                    if (i == 2) {
        ...
        """)]
    [InlineData("nested loops breaking out of the inner one", """
                var total = 0f
                for (i in 0 .. 3) {
                    for (j in 0 .. 3) {
                        if (j == 1) {
    ...
    [InlineData("an if with no else", """
        ...
        """)]
    [InlineData("an if whose branches both return", """
                if (level > 1f) {
                    return float4(1, 0, 0, 1)
                } else {
        ...
        """)]
    [InlineData("a repeat loop with a continue", """
                var total = 0f
                var i = 0
                repeat {
                    i += 1
    ...
    [InlineData("double precision", """
        ...
        """)]
    [InlineData("a swizzle written to in place", """
                var v = float4(0, 0, 0, 1)
                v.xy = float2(level, level)
                v.z = level
                return v
        """)]
    [InlineData("a struct built, stored and read back", """
                var ray: Ray
                ray.origin = colour
                ray.direction = colour
                return float4(ray.At(level), 1)
        """)]
    [InlineData("intrinsics over vectors", """
                ...
                return float4(c * m, 1)
        """)]
    [InlineData("integer minimum and maximum", """
                val low = min(count, 3)
                val high = max(count, 3)
                return float4(float(low), float(high), 0, 1)
        """)]
    [InlineData("a texture sampled through a separate sampler", """
