    ...
    TypeSymbol ResolveType() {
        if (resolving) {
            binder.Diagnostics.Add(
                SemanticDiagnostics.CircularDefinition, Declaration.Identifier.GetLocation(), Name);
        ...
        try {
            ...
            binder.Diagnostics.Add(
                SemanticDiagnostics.MissingTypeOrInitializer, Declaration.Identifier.GetLocation(), Name);
            return ErrorTypeSymbol.Instance;
        }
        finally {
            resolving = false;
