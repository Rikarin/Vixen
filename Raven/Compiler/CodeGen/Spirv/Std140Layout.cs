...
public static class Std140Layout {
    ...
    public static int Alignment(IrType type) => type switch {
    ...
    public static int Size(IrType type) => type switch {
        IrScalarType scalar => ScalarSize(scalar),
    ...
    static int StructSize(IrStructType structType) =>
        Members([.. structType.Fields.Select(f => f.Type)]).Size;
