...
internal class SyntaxToken : GreenNode {
    ...
    public override Vixen.Raven.Syntax.SyntaxNode CreateRed(Vixen.Raven.Syntax.SyntaxNode? parent, int position) =>
        new Vixen.Raven.Syntax.SyntaxToken(this, parent, position);
...
internal sealed class SyntaxIdentifier : SyntaxToken {
...
internal sealed class SyntaxTokenWithValue<T> : SyntaxToken {
    readonly T value;

    internal SyntaxTokenWithValue(
        SyntaxKind kind, string text, T value, GreenNode? leading = null, GreenNode? trailing = null
    )
        : base(kind, text, leading, trailing) => this.value = value;
