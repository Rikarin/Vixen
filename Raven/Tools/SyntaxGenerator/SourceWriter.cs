...
class SourceWriter(TextWriter writer, Tree tree, CancellationToken cancellationToken = default)
    ...
    void WriteType(TreeType node) {
        ...
        if (node is AbstractNode abstractNode) {
            ...
            for (var i = 0; i < nodeFields.Count; i++) {
                ...
                if (IsNodeList(field.Type)) {
                    WriteLine(
                        $"public {field.OverrideOrNewModifier}{field.Type} {field.Name} => new(GetRed({i}));"
                    );
    ...
    void WriteUpdateMethod(Node node) {
        ...
        Write(
            CommaJoin(
                node.Fields.Select(f => $"{(IsNodeList(f.Type) ? f.Type : GetFieldType(f))} {CamelCase(f.Name)}")
            )
        );
        Write(")");
        OpenBlock();
    ...
    void WriteFactory(Node nd, bool withSyntaxFactoryContext = false) {
        ...
        Write(
            CommaJoin(
                ...
                nodeFields.Select(f => IsNodeList(f.Type) ? $"{CamelCase(f.Name)}.Node?.Green" : $"{CamelCase(f.Name)}?.Green")
            )
        );
        WriteLine(");");
