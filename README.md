...
solution. `./build.sh` is the entry point CI and developers share. There are thirty-seven targets:
`Clean Restore RestoreNativeDeps Compile CompileRelease Test TestOrder Coverage Pack CheckPackages
...
SampleFrame AffectedProjects AffectedTests PruneWorktrees` — with
