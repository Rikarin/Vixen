...
| ⬜ A runtime path for `.vxanim` | ⬜ | Editor/Vixen.Editor.Assets | An authored clip is imported as its own YAML under the type name `AnimationClip`, and nothing compiles it into the `AnimationClipData` that `AnimationClip.Create` bakes against a skeleton — so a game cannot load one by address. `.vxscene`, `.vxmat`, `.vxcompositor` and now `.vxvfx` have all made this move; `.vxanim` is the last of what `NativeFormatImporter` calls "the ones whose compiler is still owed", and `VfxImporter` is the shape the remaining one will take. Found by `Samples/13`, whose `CharacterAnimation` computes the swing its clips describe and says so |
