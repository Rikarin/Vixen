...
| **Surface cache** (cards, atlas, radiosity) | 🟡 | `Vixen.Rendering.SurfaceCache` | [plan/19](plan/19-lighting-and-global-illumination.md) § L4, arithmetic whole and exit criterion asserted. Cards fitted by dominant-axis vote (ties to the smaller index), shelf atlas with exact-size reuse (§ 6's allocation-and-residency, CPU half), traced capture, direct sun behind a shadow ray, and the double-buffered radiosity gather — skylight arrives through the gather's misses, nothing double-counts. The Cornell box converges against a five-bounce Halton path tracer within a stated five per cent (measured 1.2), the second bounce measured as the red wall's tint arriving on the floor (ratio 1 → >1.1), and two convergences agree to the bit. `SurfaceCacheRadiance` closes the seam every tracer left open: hits answer with cached radiance and the L2/L3 probes inherit multi-bounce light without changing a line. Owed: the rasterising runtime capture, lighting/bounce as dispatches, a spatial index for sampling, and the kernels' Raven half |
