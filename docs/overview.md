...
| Constraint authoring — the track, the generated inspector, templates, the ladder, proposals | 🟡 | Core/Vixen.Animation · Editor/Vixen.Editor.Assets · Editor/Vixen.Editor.AssetEditors | Doc 34's P7, **partially**: the authoring model is complete and tested and the two largest views are not built — see the ⚠ below, and the exit criterion is not met. What is here: a `.vxanim` carries a `constraints:` block that round-trips, is checked at import and bakes against whichever rig plays it — which closes the gap P3 flagged. `GoalKindSchema` is what the inspector is generated from, so a position goal shows no aim axis; **a test walks the record and fails if a field is neither on a panel nor deliberately hidden**, and a second fails if a field the schema names has no accessor — those two are the whole mechanism against drift. `.vxpriorities` makes a priority a *name*, because a raw integer has no meaning across a project and two authors pick 70 and 700 for the same intent. `.vxconstraints` templates carry a revision and re-apply with a **diff first**, which calls out hand edits separately from template changes because a re-apply destroys them. `ConstraintProposals` watches a clip play and proposes contacts from proximity — never applied silently, and confidence is a *product* of near, still and long so any one being bad sinks it. `ProxyShapeAudit` and `MoveCoverage`/`MoveExplanations` are the two reports that say when a set is finished: **"why did it pick that clip" is otherwise unanswerable**, and a name in one body's set and missing from another's is the failure nobody can see by reading either. ⚠ **A tag is two draggable keys and not a drawn bar with its ease ramps on it** — `Timeline` is key-based and has no span to draw on, and a second timeline widget is larger than the rest of this view. ⚠ **Not built: the viewport gizmos and the error readout, the move set table, the proxy shape editor, and the sequencer's authoring-context role.** The logic each of them needs is here and tested; the panels are not, so a non-programmer cannot yet mark up a clip unaided. ⚠ The overlap audit exempts only *adjacent* joints — exempting every ancestor exempts almost everything, because a hand is a descendant of the spine and a hand buried in the belly is the one thing worth reporting |
