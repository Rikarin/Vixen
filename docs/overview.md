...
| **Ray tracing** (BVH reference) | 🟡 | `Vixen.Rendering.RayTracing` | [plan/19](plan/19-lighting-and-global-illumination.md) § L6, opened reference-first: a median-split triangle BVH — deterministic builds, front-to-back traversal, two-sided Möller–Trumbore with the geometric normal facing the ray — held against brute force hit for hit at a measured four-times-fewer visits. Owed: the RHI acceleration-structure concept behind `HasRayTracing`, and the `IDistanceFieldSource` implementation that makes "nothing above it changes" true on a device |
...
| **Built-in features register like plugins** — `PluginHost.Activate(id, name, module)`, `PluginContext.FindMenu`/`AddSubmenu` | 🟡 | Editor/Vixen.Editor.Plugin · Editor/Vixen.Editor.Blockout · Editor/Vixen.Editor.Terrain | [36](plan/36-an-extensible-editor.md) § P3. **Blockout and Terrain are through the front door**: `BlockoutModule` registers the mode and doc 24's five Scene submenus, `TerrainModule` two modes, five panels and the session that binds them to a scene — 1,460 lines out of the application, and neither assembly can see it. Terrain needed two extension points that did not exist: `PluginContext.OnUpdate` (a brush follows the entity selection and nothing raises an event about that) and `EditorDocument.Saved` (a scene names a heightfield and a foliage file beside itself). ⚠ **The reference is still in the csproj**, because `Vixen.Editor.App` is the executable and cannot dereference what it has to instantiate — that is one line each, after the exe splits off, and it is the *last* step rather than the first. The diagnostics pair and the asset editors are the remaining decoupling |
...
| vixenengine.org — the site | ✅ ([www/](../www)) — Angular 22 on xUI 2.1.0, 3 999 routes prerendered to Cloudflare static assets, readable with JavaScript off. Code, prose, outline, nav tree and palette are all `@xui` packages; nothing is a stand-in. Landing page, per-page descriptions and canonicals, `sitemap.xml` and `robots.txt` |
