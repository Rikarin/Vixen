...
| Players: `CharacterMovement` (P1), the first/third-person rigs (P2), `PlayerMoveInput` + `PlayerSpawner` (P3) | ⬜ | — | [29](plan/29-players-and-possession.md). **`MoveIntent` is written and nothing reads it**, deliberately: the seam is the risky decision and it is reviewable before anything is built on it. P3 is gated on a determinism test from P1, not a date — a movement step that is not a pure function of world and input mispredicts on every snapshot over a perfect connection, and it looks like jitter rather than like a bug |
