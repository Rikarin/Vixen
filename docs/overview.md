...
| **Built-in features register like plugins** — `PluginHost.Activate(id, name, module)`, `PluginContext.FindMenu`/`AddSubmenu` | 🟡 | Editor/Vixen.Editor.Plugin · Editor/Vixen.Editor.Blockout | [36](plan/36-an-extensible-editor.md) § P3. **Blockout is through the front door**: `BlockoutModule` registers the mode and doc 24's five Scene submenus through `PluginContext` and asks for its four host services through `PluginServices`, and its assembly cannot see the editor's application at all. ⚠ **The reference is still in the csproj**, because `Vixen.Editor.App` is the executable and cannot dereference what it has to instantiate — that is one line each, after the exe splits off, and it is the *last* step rather than the first. Terrain, the diagnostics pair and the asset editors are the remaining decoupling |
