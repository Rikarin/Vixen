...
| **Screen probe gather** (octahedral map, lattice, reference) | 🟡 | `Vixen.Rendering.ScreenProbes` | [plan/19](plan/19-lighting-and-global-illumination.md) § L3, started. The device-free half: the octahedral mapping on the Raven fold with *exact* per-texel solid angles (clipped against the octants, summing to 4π at double precision), the 16-px probe lattice with clamped bilinear reads and atlas addressing, per-probe validity (a sky pixel is invalid, not black), and a deterministic one-ray-per-texel reference gather checked against the uniform- and linear-sky closed forms. Owed: everything else — adaptive placement, importance sampling, the HZB-first trace order terminating in the field, the denoiser, and all device work |
