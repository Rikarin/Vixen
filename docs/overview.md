...
| System that marks dirty modules once a frame | ✅ | Core/Vixen.Net.Engine | `SyncStateSweepSystem`, in `LateUpdate` after `BehaviorLateUpdateSystem` — the ordering is the correctness argument and is asserted off `SystemGraph.Plan`. ⚠ **Built and unadopted**: `NetworkBehaviour` has no production subclass, and cannot get one where [#133](https://github.com/Rikarin/Vixen/issues/133) proposes — no sample references both `Vixen.Engine` and `Vixen.Net`, so nothing in the tree has both a behaviour loop and a session |
