...
| Players: `PlayerMoveInput` + `PlayerSpawner` (P3), `Samples/05-PlatformerGame` (P4) | ⬜ | — | [29](plan/29-players-and-possession.md). P3 is gated on P1's determinism test rather than on a date — a movement step that is not a pure function of world and input mispredicts on every snapshot over a perfect connection, and it looks like jitter rather than like a bug |
