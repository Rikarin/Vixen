...
| `TextEditor` model — IME, caret affinity | 🟡 | Core/Vixen.Ui · Core/Vixen.Ui.Controls · Platform/Vixen.Platform.Ui | **IME composition is built and wired**: `TextCompositionEvent`, `PlatformInput`'s `TextEditing` arm — ⚠ which was missing, so two platform heads produced a pre-edit nothing in `Vixen.Ui` had ever seen — and `TextField`'s pre-edit, which is displayed at the caret, underlined, and deliberately not in `Value`. **Caret affinity is still owed**: it is what makes bidi hit-testing answerable at a direction boundary, where one index names two places, and nothing in the tree carries it |
...
| 44 | `Vixen.Ui.Markup` | CLI path emitting generated C# to disk (`bind:` update events landed 2026-09-02) | [#181](https://github.com/Rikarin/Vixen/issues/181) |
...
| 47 | `Vixen.Ui.Controls.Advanced` | ~~Undo~~ (`CodeBuffer` states outright that it has no stack, and the editor's does subscribe — see §1.7); `GradientEditor` has no consumer; `CodeEditor` caret blink (wrap landed 2026-09-02); `AppendChild` O(n)… | [#184](https://github.com/Rikarin/Vixen/issues/184) |
