...
| 2005 | Debug | `The Vulkan pipeline cache was seeded with {Bytes} bytes from '{Path}'.` — the count is the measurement of the feature, because a blob the driver accepted is work it does not redo and a millisecond budget measures the machine | 0.1.0 |
| 2006 | Debug | `The pipeline cache at '{Path}' was discarded: {Reason}.` — ordinary: a driver update changes `pipelineCacheUUID` and invalidates every blob written before it. Handing a foreign blob back is undefined behaviour rather than an error code, so it is discarded rather than tried | 0.1.0 |
| 2007 | Warning | `The pipeline cache at '{Path}' could not be read ({Error}), so this run starts cold.` | 0.1.0 |
| 2008 | Warning | `The pipeline cache at '{Path}' could not be written ({Error}), so the next run starts cold.` | 0.1.0 |
...
| 7007 | Warning | `The query container '{Container}' never settled: it measured {Width}×{Height} on the last of {Passes} layout passes and its box was still moving.` — a query container whose verdict moves its surroundings (a flex line it wraps onto, a track it resizes) and whose surroundings then move it back, a loop the settle budget cuts rather than resolves. Its own contents cannot close that loop: `container-type` applies size containment | 0.1.0 |
...
| 7010 | Warning | `'{Element}' is a <{Control}> under a tag of its own, so it matches none of the rules for <{Control}> and has none of what they declare: {Properties}.` — a control put under another tag (`tag=` on a capitalised markup tag, `Add<T>("some-tag")`) whose computed style lacks a property its own tag resolves. Not a refusal: the rename is sanctioned and nothing was dropped, but a control's own user-agent rule is keyed on its `TagName`, so for a `ScrollView` the clip and the bars' anchor go with it. The own tag is resolved bare — no parent, classes or state — and compared by presence, so it errs towards silence. Named by element, once per distinct box, from the per-element style pass; `RetaggedControlTests` is the build-time half and sees only committed literal tags (#1327) | 0.1.0 |

⚠ **7007, 7009 and 7010 are not refusals and are the only events in this range that are not.** For 7007 nothing was dropped: the
...
measured box and a verdict can move what surrounds that box. `UiDocument.Settled` has reported that
since the wiring landed, and reports it about the *document* — which is not a thing anybody can go and
fix in an interface with a dozen containers. This names the box. ⚠ The loop through a container's
*own contents*, which this event was first written for, no longer happens: `container-type` applies
size containment (doc 43 § A16's coercion), so a container's contents cannot size it on the axis it
answers. What still reaches this event is a container its surroundings move.
...
| 13033 | Warning | `--vixen-capture was given but this run has a surface to present to, so nothing will be written to {Path}. Add --vixen-headless (⚠ not --vixen-offscreen, which chooses the device and not the surface), or leave AppConfig.Window null.` — ⚠ the other way to ask for a picture and get none, and the quieter one: 13029's run has no frame to capture, this one has a frame and nowhere to read it from. The flag names are the whole point of the message — `--vixen-offscreen` decides which *device* opens and touches neither the window nor the surface, so a desktop run given it lands right back here | 0.1.0 |
| 13034 | Information | `Log configuration from {Path}: {Rules} category rule(s), minimum {Minimum}.` — ⚠ said even when the file agreed with the defaults, because a configuration file nobody read is otherwise indistinguishable from one that was read and changed nothing; a run whose log does not carry this line found no `vixen.log.yaml` at all | 0.1.0 |
| 13035 | Warning | `Log configuration at {Path} was ignored: {Reason}` — a mistyped level or an unreadable file, and a warning rather than a stop: the alternative is a game that refuses to start because of the one file whose whole job is to make the game easier to watch | 0.1.0 |
| 13036 | Information | `Wrote a Chrome trace to {Path}: {Samples} sample(s) over {Frames} frame(s), {Dropped} dropped.` — `--vixen-trace`; ⚠ the dropped count is on the line because a profiler ring overwrites rather than grows, so a long run's trace is its most recent history and a reader who does not know that will quote a total from a document missing the middle | 0.1.0 |
| 13037 | Warning | `The trace could not be written to {Path}: {Reason}` — a warning rather than a stop, and said at shutdown: everything the run was for has already happened, and refusing to exit over a file nobody can write helps nobody | 0.1.0 |
...
| 14019 | Warning | `Nothing is published at '{Address}', so the sample has no morphed head to show and the foreground is empty. …` — `Samples/03` with no content build | 0.1.0 |
| 14020 | Information | `The heads: {Shapes} shape(s) bound, weights [{Weights}] from '{Clip}'; the pre-pass held {Meshes} mesh(es) and {Instances} instance(s), and in the last frame copied {Copies} and dispatched {Dispatches} ({Dropped} dropped).` — ⚠ the line that would have caught the morph pre-pass never running: every other counter a morphed frame reports reads healthy when it did nothing, because the mesh is still extracted, shaded and shadowed, just at rest | 0.1.0 |
...
| 14056 | Information | `VSM: {Marked} page(s) marked by the last serviced frame, {Absent} of them absent from the table this frame publishes, {Drawn} drawn this frame, {Invalidated} thrown away by the refit, {Resident} resident in {Slots} slot(s) after {Allocations} allocation(s).` — `Absent` is the only coverage number; the rest are effort, and an absent page renders as the cascade fallback | 0.1.0 |
...
| 14072 | Information | `Buoyancy: {Floating} bod(ies) floating, {Pontoons} pontoon(s), {Wet} of them wet, {Submerged} of each under on average; the deck is at {Deck} over a surface at {Surface}, an offset of {Offset} m. The overlay has drawn {DebugFrames} frame(s).` — pontoons above zero with none of them wet is a body outside every zone's window; debug frames at zero with `water.showBuoyancy` on is `BuoyancyDebugSystem` unwired | 0.1.0 |
| 14073 | Information | `Console: typed '{Line}'; claimed = {Claimed}.` — `VIXEN_CONSOLE`; false is a verb nothing registered, and a typo reads the same, which is why the line is echoed rather than summarised | 0.1.0 |
| 14074 | Warning | `VIXEN_CONSOLE said '{Script}' and this run has no console: pass --vixen-overlays, which is what builds the console, the debug accumulator and the node that draws it.` | 0.1.0 |
| 14075 | Information | `Claimed {Casters} of the level's drawable(s) as static shadow casters, so the sun's cache keeps them. Zero with a staticStage: in the document is a cache that costs a copy and saves nothing.` — the scene's half of `ShadowMapRenderer.StaticCasterStage`; a level with none of these looks exactly like a level with all of them | 0.1.0 |
