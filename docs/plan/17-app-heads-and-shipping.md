...
| Q5b | May a Release build load loose files behind a flag? | ✅ **Decided: yes.** `--vixen-loose-content <path>`, off by default, refuses silently-quiet operation. Weakens the "release reads only bundles" invariant in exchange for diagnosability — the trade is deliberate and visible, and the visibility is the half the trade was made for. **Two of the three surfaces exist**: `VixenApplication.WarnAboutLooseContent` repeats `HostLog.LooseContentStill` every 60 s (once at startup is not visible — a build left running overnight scrolled that line away hours ago), and `DiagnosticOverlays` draws a `CONTENT LOOSE` standing notice in the alarm colour, which is the surface a screenshot in a bug report carries. ⚠️ **That notice is not an overlay, and the difference is the whole of [#919](https://github.com/Rikarin/Vixen/issues/919)**: it was a row on `FrameStatsOverlay`, and `overlay stats off` at the console erased it — as did `Remove`, `DisableAll`, and `Enabled = false` written straight onto the overlay, which the registry never sees. A build that can be made silent again by one console command is not the visible half of the trade. It is still inside `DiagnosticOverlays.Enabled`, which asks for no diagnostics at all and is a different request. ⚠️ The crash-report stamp is blocked on crash reports existing at all ([#331](https://github.com/Rikarin/Vixen/issues/331)) and belongs in whatever lands there. |
