...
dotnet new vixen-plugin    # ⬜ editor plugin — needs Vixen.Editor.Plugin to exist
...
every template is two copies waiting to disagree. `vixen-plugin` is blocked rather than pending: it
would pin a `PackageReference` on an assembly nobody publishes, and a template that produces a
project which will not restore fails at the one moment a person has no context to debug it.
...
| Q5b | May a Release build load loose files behind a flag? | ✅ **Decided: yes.** `--vixen-loose-content <path>`, off by default, refuses silently-quiet operation: it logs a warning banner every 60 s and stamps the diagnostic overlay and crash reports with `LOOSE CONTENT`. Weakens the "release reads only bundles" invariant in exchange for diagnosability — the trade is deliberate and visible. |
