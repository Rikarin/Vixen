...
| `TestOrder` | prints the order `Test` starts the test assemblies in, which is longest first out of `build/test-cost.txt`; `--update-test-cost` rewrites that list from the last run's TRX `Times`. ⚠️ **What used to decide the order was `Vixen.slnx`**, and a solution build of a custom target walks each project's dependencies first — so the assembly referencing the most of the tree was dispatched last, and that assembly is for the same reason the slowest one. `Test` hands MSBuild a generated flat traversal project instead. Measured on the 2026-09-05 runs either side of the change: elapsed 873.3 s → **677.5 s**, with `Vixen.Editor.App.Tests` moving from a 218.4 s start to 0.2 s ([#592](https://github.com/Rikarin/Vixen/issues/592), then [#557](https://github.com/Rikarin/Vixen/issues/557)). ⚠️ **This row used to end "the run is now its longest single assembly and cannot be shortened by scheduling at all", and that expired when #557 halved that assembly** — 655.0 s → **329.5 s** against a 498.3 s run in the 2026-09-05 23:04 TRX, so 169 s of the run happens after it finishes and greedy LPT bottoms out at six workers rather than at four. The cost list itself was the last thing holding the old conclusion up: it still said 655.0, and a schedule input wrong by 2× is wrong in the direction that makes the run look unfixable. ⚠️ **A stale number *can* fail `Test`, and for most rows it cannot** ([#863](https://github.com/Rikarin/Vixen/issues/863), corrected by [#1128](https://github.com/Rikarin/Vixen/issues/1128)): the same pass that counts the TRX afterwards compares each measured wall with the committed cost and fails on a gap over **both** 60 s and 1.5× — both, because the small assemblies are host start-up and noisy in ratio while irrelevant in seconds, and 1.5 rather than 2 because the drift it was written for was 1.988×. ⚠️ **This row said "a stale number now fails `Test`" without the qualifier, and that is the overclaim #1128 is about**: the 60 s floor means a row measured under a minute can be wrong by any amount and never fail, which is most of the list. The check is a guard on the assemblies that decide the schedule, not on the file. It fails only when the two are the same measurement: `--update-test-cost` stamps the configuration into the file's header, so CI's Release run on other hardware reports the gap and declines to fail on it. ⚠️ **And the gap is measured after the run's own load is divided out** ([#938](https://github.com/Rikarin/Vixen/issues/938)): the first time the check fired it named the five *longest* assemblies at a uniform ~2.2×, every one of which measured its committed cost to within half a percent when run alone, because five worktree agents were compiling on the same box — so each row is now compared with its committed cost times the median of measured-over-committed across the rows above 30 s. A machine multiplies the whole list and a drift moves one row. ⚠️ **What that cannot separate is a uniformly stale list from a uniformly busy machine**, which is the same measurement by wall clock; the scale is reported and never failed on, and the axis that does speak to it is the `# measured:` date |
...
| `GoldenImages` | ✅ renders the fixture suite on the local backend — lavapipe on the Linux leg, MoltenVK on macOS — and compares it with the committed references; writes the rendering, the reference and a diff into `artifacts/golden-diff/` on failure. `--update-golden` rewrites the references. The fixtures also run under `Test`, so a wrong picture fails an ordinary build; the separate target exists for the diffs and the switch. ⚠ **`--update-golden` rewrites only the references that failed** — it used to rewrite every fixture it rendered, and because the comparison is *tolerant* rather than bitwise that silently re-accepted drift a passing reference had already absorbed (`tier-low` sat at 36% of its mean allowance for a month, [#1242](https://github.com/Rikarin/Vixen/issues/1242)). `--force-golden` is how somebody re-records a *passing* reference on purpose, which is a real workflow: a dither that moves every pixel without failing anything leaves a reference that is not what the frame looks like. Either run now writes `golden-update.tsv` beside the diffs, a line per fixture saying recorded-or-kept and what it spent of its tolerance; a checking run writes the same as `golden-headroom.tsv`. Both are reports and neither is a gate — on the day nothing ran, there is no file, which cannot be mistaken for health. ⚠️ **This row used to say `artifacts/golden-diff/` is "what CI uploads", and it is uploaded but never written there**: no CI job runs this target at all, and `artifacts/golden-diff/` exists only because `GoldenImages` sets `VIXEN_GOLDEN_DIFF` to it. The diffs a CI failure actually carries are the `Test` leg's, where that variable is unset and `GoldenImage.DiffDirectory` falls back to the directory beside each test binary — which is why `ci.yml`'s upload step names a second path, `**/bin/Release/net10.0/golden-diff/`, and that is the one with files in it. With `if-no-files-found: ignore` on the step, the empty half is silent. |
...
`--force-golden`, `--coverage-project`, `--remove-merged`, `--benchmark-filter`, `--short`,
...
nuke GoldenImages --force-golden                       # ⚠ and the ones that passed too, on purpose
