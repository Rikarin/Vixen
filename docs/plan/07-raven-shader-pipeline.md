...
| ✅ | **An expression statement that cannot do anything** — `RVN2141`. An expression statement's value is discarded, so unless evaluating it does something the statement is not there; only an assignment, a call and an increment can. It went unreported because nothing asks what a value at that position is *for*, which is the same hole `RVN2140` came out of — and it made a **silent miscompile out of layout**, because a newline ends a statement: `x = x` on one line and `+ y` on the next are two well-formed statements, and the sum is not there. Three shipped shaders were in that state, one of them found only by a rendered picture. The mirror layout, trailing the operator, was already `RVN1001` — which is what made the arrangement that says nothing the one an author reaches for |
