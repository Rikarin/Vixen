...
| A shader language with compute | [`Raven`](../../Raven/README.md) — `Raven/README.md:592`, `:627` | Workgroup size on the stage attribute, `RWBuffer<T>` and storage images. **A node kernel is a Raven compute shader** — ⚠ an *editor* one, which `CheckShaders` reached only through a hand-kept list of four when this was written, and since `63b750f15` reaches by walking the tree for every `.rvn` with a `.spv` beside it; see § D1 |
