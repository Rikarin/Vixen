...
| `addressable` | Address, group, labels, `excluded`. All optional. ⚠ **An asset with no `address` is shipped under its project-relative path** — `Assets/Textures/Crate.png` — which reverses this document's original "no `addressable` block means not shipped". The reason is what that cost in practice: every asset a game wanted to load by name, and every scene in the build list, was a build error whose remedy was to go and fill in a field, so most projects filled in none and discovered it on a device. Setting an address is now for the ones that are *contracts* — a level a save game names — because an explicit address survives the file moving and a path does not. `excluded: true` is how something is kept out, and a folder and a `.vxgroup` are never defaulted. |
| `subAssets` | Declared sub-assets with **stable IDs** (see below), their names, their types, and the source name where a rename or a collision moved it. ⚠ Two sub-assets of one kind with one name — two meshes called `Cube` in a `.glb`, which is what a model from anywhere but your own DCC tool looks like — no longer fail the asset: the second becomes `Cube_1` with a warning, and `importer.subAssetNames` is where the author gives it a better one. Explicit, typed, and human-readable, replacing `internalIDToNameTable` + `mainObjectFileID`. |
