...
| Light probes (SH, tetrahedral interpolation) | ⛔ | **Retired by [19](19-lighting-and-global-illumination.md).** Attempted and withdrawn — Bowyer–Watson needs exact predicates to survive a cospherical grid of probes, which is what a grid of probes is. Doc 19 replaces it with an irradiance field on a brick-and-indirection volume (L2): integer arithmetic and hardware trilinear, no predicates, and it cannot fail the same way. The SH side either would feed is built and tested |
