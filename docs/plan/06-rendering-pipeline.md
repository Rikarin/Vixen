...
| Output dithering | 🟡 **written, and no shipped frame turns it on** | ⚠ **Split out of the row above, where it was hidden inside a ✅.** What stops banding in a gradient on an 8-bit display, and the cheapest thing in the whole post chain. `Vignette.rvn` declares `UseDither` with `ditherLevels` and `ditherIntensity`, applied last in the fragment — after the tonemap and after the grain, because it exists to break up the quantisation step of the final encode and dithering before a curve dithers the wrong quantity. The noise is the difference of two independent per-channel hashes, so it is triangular over ±1 code, and it is *static* rather than per-frame: the banding is spatial, and a moving pattern would add crawl to a still image to fix what a fixed pattern fixes. `DitherImageTests` holds it on a device against a ramp one code wide — the density has to track the ramp *and* the frame's mean must not move, because "the image changed" is satisfied by any noise including a bias ([#1156](https://github.com/Rikarin/Vixen/issues/1156)). ⚠ **`!StandardFrame` does not set it**, and that is a finding rather than caution: `ditherLevels` divides the value the *shader* outputs, which is one stored code only on a target with no transfer curve, and the lens node takes `VignetteAsset.Format`'s `Rgba8UNormSrgb` default — so a fixed 1/255 is about forty-five times too much noise in the shadows, which is exactly where banding is worst ([#1181](https://github.com/Rikarin/Vixen/issues/1181)) |
