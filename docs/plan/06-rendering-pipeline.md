...
| Bent normals consumed by the ambient term | ⬜ | ⚠ **Split out of the SSAO/GTAO row, which records it inside a ✅ as "a permutation nothing consumes".** GTAO computes the bent normal for free as part of the arc integral and `Ssao.rvn`'s `BentNormal` permutation writes it as a view-space direction; what is missing is a reader. `AmbientCombine.rvn` samples the contact plane's `.r` and nothing else, so the ambient term still uses the geometric normal — which is most of what makes GTAO look better than SSAO on curved surfaces. ⚠ **This is doc 43's `--scale` shape in the renderer: an output nothing reads, which no gate can tell apart from an output nothing produces**, because both are green. [#1157](https://github.com/Rikarin/Vixen/issues/1157) |
