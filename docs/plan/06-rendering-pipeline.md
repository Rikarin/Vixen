...
| MSAA (**classic path only**) | P1 | 2/4/8×, with a custom depth resolve (Stride has `MSAADepthResolverShader`). ⚠ Not available on the virtualized-geometry path, and that is a decision rather than a gap: a visibility buffer breaks MSAA — per-sample visibility is four times the buffer and a per-sample resolve, which is the trap deferred falls into. TAA is shipped and owns its history, which is what Nanite leans on for the same reason. See [22](22-virtualized-geometry.md) § Phase 5 |
