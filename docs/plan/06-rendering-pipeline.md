...
Priority column: **P1** = required for the 1.0 renderer, **P2** = post-1.0, ✅ built, 🟡 partly built
with what is missing named in the row, ⛔/✂️ deliberately not built.

### How this document is kept, and why it needs saying

⚠ **This is the one plan document whose tables are a feature *inventory* rather than a design**, which
makes it the one that goes stale silently — a design is wrong out loud the moment somebody reads it
against the code, and an inventory just quietly stops being true. Nine markers in it had rotted when
this was written and every one had rotted in the *safe* direction: the document under-claimed, which is
why none of them had caused a bug.

**The safe direction is not the guaranteed one.** The same mechanism rots the other way just as
easily, and this document has already been caught doing it once — the punctual-shadow row read ✅ while
the atlas was **read by nothing**, so every spot and point light in every scene lit through walls while
the inventory said the feature shipped.

The rule that would have prevented all nine, and it is cheaper than any gate: **the row is updated in
the commit that closes it.** Doc 43 solved the same problem by *generating* its state column and
failing the build on a hand edit; that is not proportionate here.

⚠ And where this file disagrees with `docs/overview.md`, **`overview.md` is the state and wins.** This
one records intent.
...
| Blend shapes / morph targets | ✅ | [33 § D4](33-character-creator.md)'s design, landed as a slice: `MorphTargetData` (sparse `(index, Δposition, Δnormal)`, 16-bit snorm against a per-target range), `MorphKernel` as the arithmetic written once, `Pipeline/MorphScatter.rvn` as its transliteration, and `ModelReader` reading `aiMesh.mAnimMeshes` behind `importBlendShapes`. Held together on a device by `MorphScatterDeviceTests`. ⚠ **One dispatch per active target with a barrier between**, which amends § D4's "one dispatch per instance": two shapes may move one vertex and Raven has no float atomic. `MorphRenderFeature` is the render feature and it landed: a vertex range per morphed instance, the rest pose copied into it out of the scene's `GeometryBuffer`, a dispatch per active shape, and `MeshDraw.VertexBuffer` **and** `VertexOffset` rewritten together — wired in `WorldRenderer` and fed by `BlendShapeWeights` + `MorphWeightSystem`. **And a clip drives one:** `AnimationChannel` carries a fourth, scalar track (`Shape`/`WeightTimes`/`Weights`), `ModelReader` reads it out of `aiAnimation.mMeshMorphChannels`, `AnimationClip` bakes one flat track per shape with the bucket index the vector tracks get, `ClipMotion` collects it into `Animator.MorphWeights` scaled by the clip's blend weight, and `BlendShapeAnimationSystem` lands it on the component. ⚠ **By name, not by slot** — the import drops a shape under `BlendShapeThreshold` and deduplicates the rest, so a source file's ordinal is not `MeshData.MorphTargets`', and `MorphWeightSystem` publishes the translation out of what the feature attached. ⚠ **Two versions moved — `ModelImporter.Version` 9→10 and `AnimationClipContent.Current` 1→2 — as re-import triggers and not compatibility fences.** The generated serializer refuses only `count > MemberCount`, so appended members read out of older bytes as defaults and an old artefact truthfully answers "no weight track"; nothing breaks without the bump, and nothing *happens* either, because the curves were dropped at import and only a re-import goes back to the file for them. **Owed:** hand-authoring a weight curve (`AnimationProperty` has no `Weight`). ✅ **And the paged path morphs, as of 2026-08-27** — the row above called it the *cluster-page scatter* and a scatter is the one thing it cannot be: a page is per **mesh** and every instance reads the same bytes, while weights are per instance, so there is nowhere for a per-instance scatter to write. `ClusterRaster`, `ClusterSoftwareRaster` and `VisibilityResolve` **gather** a vertex's shapes where they decode it, exactly as they already gather an instance's bone palette; `MorphIndex` is the table, built at registration out of `MeshData.MorphTargets` and `MeshletMesh.Vertices` rather than as an artefact. ⚠ **A gather needs no barrier and has no race**, which is the one thing it is strictly better at. ⚠ **A page vertex does not know which mesh vertex it is**, so it costs two indirections — and the happy consequence is that every copy of a locked boundary vertex resolves to one source vertex and picks up one delta, so the cut stays crack-free through an expression. `ModelImporter.Version` 12 builds a hierarchy for a morphed mesh again. Proved by a picture rather than a counter — `VirtualGeometryGoldenTests.A_virtualized_mesh_moves_where_its_blend_shapes_say`, on a device — because a counter is what said this feature was fine the first time |
...
| Impostors / billboards | ✅ (bake and draw) | ⚠ **Promoted, and given its consumer.** This is not a general feature that terrain happens to want — it is the last two hundred metres of a forest, it has no other caller, and building it without one would be guessing at its requirements. An octahedral bake from the mesh the asset pipeline already has, reusing `MeshSimplifier` for the intermediate levels rather than a second simplifier. [31 § T7](31-terrain-grass-and-trees.md#t7--impostors-and-the-far-field--10-em). ⚠ **This row read P1 long after it landed.** `ImpostorGrid` folds the directions onto a hemi-octahedron, `ImpostorAtlas` places them, `ImpostorCapturePass` draws a mesh into a cell and `ImpostorBake` drives the whole bake, over `Terrain/ImpostorCapture.rvn`, `Terrain/ImpostorFinish.rvn` and `Terrain/Impostor.rvn`; `ImpostorBakeTests`, `ImpostorCapturePassTests` and `ImpostorTests` hold it, and `docs/guide/rendering/impostors.md` is the guide. ⚠ **What the row promised and the tree does not have is the consumer it was promoted for**: nothing in `Vixen.Rendering.Terrain` or in the foliage shaders names an impostor, so the far field still draws meshes. That half is 31 § T7's, not this document's |
| Sprites, sprite sheets, 9-slice | ✅ | `SpriteRenderFeature`, and what it shares with the interface is `NineSlice` (`Vixen.Core.Mathematics`) — the cut of a rectangle into nine, which `UiGeometryBuilder` stretches a panel's background with and this stretches a sprite with. More was not shareable: `Vixen.Ui` describes a frame without a device and `Vixen.Rendering` draws without knowing what an element tree is, so what they have in common is the arithmetic and the shape of the answer — quads into one buffer, one binding, a draw per object reaching its run through the vertex offset. **Local space, not camera-facing**: a sprite is a quad in its own plane and where it sits is `TransformRenderFeature`'s matrix, so the geometry is built once a frame rather than once a view — which is what separates it from the particle expansion, and why a billboard is still that feature's job. `Sprite` is texels plus a pixel density, `SpriteSheet` cuts and names them, `SpriteAnimation` is indices sampled by time rather than stepped. ⚠ **Tiling is here and not in `Vixen.Ui`**, and the reason is what decides the split: a repeat count is destination ÷ natural size and the natural size is in texels, which a draw list does not know — so a nine-slice in the interface is stretched only, and one in a scene can repeat. Capped at `SpriteGeometry.TileLimit`, above which a cell is stretched instead, because the count is a property of how small the artwork was drawn and not of the scene |
| Decals (deferred + forward clustered) | P2 | |
| Terrain (~~clipmap~~ quadtree morph, virtual texture) | P1 | ⚠ **Promoted, and half of this row is rejected on the merits.** A geometry clipmap is the wrong structure for an *authored, edited* heightfield: its rings are centred on the camera, so they align with none of the rectangles that matter — the tile, the collision shape, the streaming page, the undo rect — and the mapping from a ring texel to a terrain sample changes every frame the camera moves. The engine already runs a clipmap for the case it *is* right for, in `Vixen.Rendering.DistanceFields`. Terrain is CDLOD instead: a quadtree, one instanced grid patch per selected node, and a vertex morph towards the parent level that removes cracks without skirts. The virtual texture is **deferred rather than dropped** — an eight-layer terrain touches two or three layers per pixel through one bindless array, which is a normal material, and a page table is a second failure mode bought before it is needed. [31 § D3](31-terrain-grass-and-trees.md#d3-a-quadtree-with-a-morph-not-a-clipmap), [§ D7](31-terrain-grass-and-trees.md#d7-no-virtual-texture-in-the-first-pass-and-the-loop-is-why) |
...
| Area lights (rect/disc/tube) | ✅ (representative point) · LTC ⬜ | Sphere, tube and rectangle, through Karis's **representative point** rather than LTC: shade the point on the shape nearest the reflection ray and widen the lobe by the angle the shape subtends. A rectangle also takes its own cosine, which is what makes it a panel rather than a glowing slab. ⚠ **Not LTC, which this row asked for.** LTC replaces two approximations — a highlight with the right size and the wrong shape, and a diffuse term that treats a near light as a point on it — with a closed-form polygon integral, at the cost of a fitted table that comes from an offline optimisation this repository has no machinery for. The five kinds share one 80-byte record and one loop, so clustering and the per-object light list needed no second path — **and LTC has to arrive as a second `Resolve` behind that same record, not as a fork**. ⚠ **"Nothing is in its way" is the part of this row that does not survive checking.** The table is the work, and it is a licence question before it is a graphics one: `AreaLights.rvn` puts it at **two** 64×64 float textures, and Heitz et al. publish theirs with the paper and with implementations under varying terms. ⚠ **The `SmaaAreaTexture` precedent is weaker than it looks.** That table is *generated* rather than vendored — "so no licensed data enters the tree" — but it is generated because it is **analytic**: the area under a straight line clipped to a pixel column, which is arithmetic a pencil can check and `SmaaAreaTextureTests` does. An LTC table is a **numerical fit**, so generating it is running the optimisation rather than writing down a closed form, and the two are not the same kind of task. So the choice is a real one: run the fit here, or vendor a table whose licence is checked and recorded in `NOTICE` and in [01](01-technology-decisions.md) § ADR-015 the way every other reference artefact is — see [#129](https://github.com/Rikarin/Vixen/issues/129) |
...
| Light probes (SH, tetrahedral interpolation) | 🟡 | The CPU half is built: `ExactPredicates` (`Vixen.Core.Mathematics`) answers orientation and in-sphere by a filtered `double` evaluation with a `BigInteger` fallback and simulation of simplicity for the cospherical ties every grid produces, `DelaunayTetrahedralization` is Bowyer–Watson over them, and `LightProbeVolume` blends the four probes a position sits between. This row previously read *attempted and withdrawn*, and the three failures it listed — an oversized enclosure swallowing the domain, a cospherical grid finding no cavity, a near-degenerate cell eating the mesh — were one defect wearing three hats: a sign asked of floating point. What is still owed is the GPU half — uploading a volume and sampling it in a shader ⚠ It is not what [19](19-lighting-and-global-illumination.md) § L2 builds on: that field places probes on a lattice inside bricks, so a lookup is a divide and a hardware trilinear fetch and no triangulation is involved at all. The two are alternative answers to "what is the indirect light here", and L2 is the one the plan commits to. |
| Reflection probes (box/sphere projected, blended) | ✅ | Parallax-corrected against a box or a sphere, faded against the environment over the probe's own blend distance, and selected by priority then volume so a cupboard inside a room wins inside the cupboard. Selected **per object**, and it costs an `int`: the cubes are one binding with a count bound for the frame, the volumes are an array beside them, and `ForwardLightingRenderFeature` writes the index and the weight into the padding std140 already left after the light count. `SceneLighting` fills that array from the same selector in the same order — the array's length off the shader's own plan, the spare slots taking the sky's cube, since the shader samples the slot before it weighs it. ⚠ For the forward pass's own ambient term a probe still fades against the **sky** rather than against a second probe. For reflections, [19](19-lighting-and-global-illumination.md) § L5 retired the caveat: `ReflectionTrace` answers the near field through the traced slots, and `ReflectionProbeMissSource` — `ReflectionProbeMiss` on the CPU, held against it on the device — puts these probes in the miss seat, where the fade is between two kinds of far field rather than a reflection vanishing. The diffuse half is its own row below |
| A probe's *diffuse* ambient blended against a second probe | ⬜ | ⚠ **Split out of the row above, where it was the last clause of a ✅.** Doc 19 § L5 retired the *reflection* half of that caveat and not this one — and this half is a step further back than the caveat implies. In `ClusteredShading.rvn` the specular ambient does `lerp(prefiltered, Probe(…), ProbeWeight())` — one probe against the sky — while the diffuse ambient is `Ibl.IrradianceSh9(n, environmentSh)` and reads **no probe at all**: a reflection probe carries a prefiltered cube and no irradiance SH for the shader to blend. The symptom is a visible step in indirect light when walking between two probe volumes, and the fix is a second quantity per probe rather than a second lerp. [#1158](https://github.com/Rikarin/Vixen/issues/1158) |
| Shadow maps: CSM (directional) | ✅ | `ShadowMapRenderer` — **a cascade is a view**: four `RenderView`s over one stage, culled and sorted by machinery that knows nothing about shadows, into four tiles of one atlas in one pass. Crawl is fixed at its two sources: a *sphere* fit (so turning does not resize the cascade) and texel snapping (so sub-texel movement gives a bit-identical matrix). What a shading pass reads is published into set 0 by the node itself — the matrix with its atlas tile folded in, the texel size, the biases and the sampler. Selection is **per fragment**: the block holds a `ShadowCascade[CascadeCount]` — a matrix and the distance it is valid to, together — and a fragment picks the nearest one that still covers it from its own view depth. It read one matrix and one distance until then, so everything past the nearest slice projected outside its tile and came back unshadowed, which reads as a shadow distance far shorter than the setting. The last cascade's end is ramped by `Lighting.CascadeFade` rather than stopping at a line across the ground. ⚠ Two things that survived every assertion here, both found by a person turning the camera: the fit derived the light's texel grid from **`camera.Up`**, so the whole map rotated under stationary geometry whenever the player looked around — the guard's condition was the negation of the case its own comment described, so the camera-independent reference on the line above it was dead code; and the atlas fold translated y the way it translates x, which inverts the tile *row* under the negation `Transform.NdcToUv` gained after the fold was written, so cascade zero read cascade two's tile. The sphere fit says a cascade is the same *size* whichever way the camera points and the snapping says it is stable under sub-texel movement; neither says anything about its *shape*, and neither test computed a UV the way a shader does. `Rolling_the_camera_does_not_turn_the_light` and `An_atlas_matrix_lands_in_the_tile_its_viewport_drew_into` are the two that now do |
| Shadow maps: cube (point), perspective (spot) | ✅ | `PunctualShadowRenderer`. Short where cascades are long, and the reason is worth stating: a punctual light *already is* a volume, so nothing has to be invented from the camera and nothing has to be stabilised. Six 90° frusta tile the sphere exactly — asserted over ten thousand directions, because a seam in a shadow cube is light through a wall along one line. A point light is six tiles and a spot is one, and a light that does not fit is dropped **whole** and counted. ⚠ This row read ✅ while the atlas was **read by nothing**: no shader in `Raven/Library` had a punctual shadow lookup in it, so the node rendered a correct depth atlas that the frame threw away and every spot and point light in every scene lit through walls. `Raven/Library/PunctualShadows` is the half that did not exist — a compose slot, so a frame with no atlas declares no bindings and compiles the lookup to `1f` — and the light's tile index rides in `PunctualLight.shadowIndex`, written back by the node that packed the atlas so that a dropped light is unshadowed by construction. Fixing it turned up a second one in the row above: the atlas fold used the same tile translation on both axes, which inverts the tile *row* under the y negation `Transform.NdcToUv` gained after it was written, so cascade zero was reading cascade two's tile |
| Shadow filtering: PCF, PCSS, VSM option | P1 | PCF default, PCSS for soft area shadows |
| Shadow atlas + caching for static casters | ✅ | Directional cascades. Two things had to be true together: the projection has to stop moving, which `ShadowMapRenderer.Slack` buys by cutting the cascade wider than its slice and keeping it while it still covers one — trading resolution for stability, since the same texels then cover 1.5625× the area at 25%; and the static casters have to be separable, which a second `RenderStage` already is, so no filtering machinery was needed. The cache is redrawn only when a cascade re-fits or the host bumps `StaticVersion`, and `StaticRebuilds` is what makes "it caches" checkable. **Reachable from a document since 2026-08-21, and until then it was not** — the node was complete and had no field on `ShadowMapAsset`, no arm in `CompositorBuilder` and no sample: `staticStage:` and `slack:` are the document's half, `GraphicsOptions.StaticCasterStages` the host's, and the scene's is `StaticShadowCaster`, a tag `MeshExtractionSystem` stamps a different stage mask for. The cache atlas is the node's own texture, because every `!Resource` a document can declare is transient. Measured on Samples/13 under `VIXEN_WALK`: 1 624.5 → 1 407.0 draws recorded a frame, 24 rebuilds in 256 frames, same picture to 0.0001/255. ⚠ The split found two defects that were not the cache's — a shader variant keyed without the stage that imposed it, so the first of two caster stages decided what every stage drew with and an unfilled one drew with set 2 empty; and a `!VirtualShadow` node given only half the casters, which outranks the cascades wherever it has a page and therefore deletes the level's sun shadow from most of the screen while every counter reads healthy. **Punctual lights are cached too now, and on a different plan** — `PunctualShadowRenderer.Cached`: a punctual frustum does not know the camera exists, so neither the stability trade nor the static/dynamic split arises and the *whole* tile is kept. A tile is redrawn when the lamp's projection inputs changed bit-for-bit, when the casters meeting that tile's frustum changed, or when `CasterVersion` says so, with one render-area-confined pass per redrawn tile because a clear obeys the render area and not the scissor. ⚠ Its own trap, found by measuring rather than by reading: a GPU-culled frame with no readback hands the host the *conservative* set, so every tile's work list is the whole level and a cache trusting it saves nothing — the node re-tests against the tile's frustum. 108 → 4–8 tiles a frame in Samples/13 |
| Contact shadows (screen-space ray-marched) | P2 | |
| Baked lightmaps + GI bake | ⛔ | **Retired by [19](19-lighting-and-global-illumination.md), not deferred.** The single biggest saving in that document: it takes a lightmap UV unwrapper, a chart packer, seam fixing, an atlas allocator and a UV channel in the mesh format off the plan entirely. Lumen's own answer is a surface cache re-lit every frame, and doc 19 follows it |
| SSGI / RTXGI-style probes | P2 | Now [19](19-lighting-and-global-illumination.md) § L3 — screen probes on a ~16 px grid, importance-sampled, resolved to SH, with long rays terminating in L2's field |
| Distance fields: mesh bake, clipmap, tracer | ✅ | [19](19-lighting-and-global-illumination.md) § L1. `Vixen.Rendering.DistanceFields` bakes an exact field per mesh with a backface-voted sign, composites instances into a camera-snapped clipmap that scrolls rather than recomposites, and marches it. Device-free and checked against closed forms; `GlobalDistanceFieldTexture` is the GPU mirror and `GlobalDistanceFieldRenderer` drives it |
| Distance-field shadows and occlusion | ✅ | `DistanceFieldAo` — one march gives both, and the sun's penumbra falls out of how close the ray passed rather than out of a filter kernel. The companion to GTAO rather than its replacement: that one is sharp and blind off screen, this one sees the whole world at the field's resolution. ⚠ **This row's "compiles and binds on a device; no test has drawn with it" is retired.** `Platform/Vixen.Graphics.Golden.Tests/DistanceFieldAoImageTests.cs` draws with it, and it starts from the null field because that is the case with an exactly knowable answer — nothing near means nothing occluded and nothing blocking, so every pixel is `(1, 1, 0)` and a shader that did not run gives black, which is as far apart as a frame can be. Writing it found the thing worth finding: **a full-screen pass had no way to fill a compose slot at all**, so `DistanceFieldAo` could not be built by a compositor under any composition |
| Volumetric fog / lighting (froxel) | ✅ | ⚠ **"Shares the clustering grid" was half wrong and is corrected here.** What is shared is the *Z distribution* — `near · (far/near)^(k/n)`, and `ClusterGrid.DepthOf`'s negation with it — and a froxel's light-list lookup through `ClusterGrid.Of`. What is not is the grid itself: 16×9×24 with a light capacity of 32 is far too coarse for fog, and it is a **structured buffer**, not a 3D texture, so nothing can filter a froxel against its neighbours in it. `VolumetricFog.rvn` owns its own volumes at its own resolution over **its own far plane** — 64–100 m, never the camera's, because a grid stretched over a camera's range spends almost every slice where the analytic falloff already suffices and leaves the near slices too thick to hold a beam's edge. Three compute passes behind one `Pass` permutation (inject the medium, light it, march it) plus a `Volumetric` permutation on `Fog.rvn` that composites the volume and lets the analytic term carry on *beyond* the far plane, measured from the far plane rather than from the camera. Landed: sun and sky in-scatter, height falloff, the tier knobs, the `!StandardFrame` seats, and the local `PostProcessSettings` overrides — where ⚠ `null` is "no opinion" and `0` is "no fog *here*", which is how a cellar clears the mist a level-wide volume filled. ⚠ **This row's "Owed: shadowed in-scatter, the per-froxel punctual light loop through `ClusterGrid.Of`, and temporal reprojection" listed three things that had all landed.** All three are permutations of `Raven/Library/PostFx/VolumetricFog.rvn` — `Shadowed` reads the cascades the scene pass published and takes `FrameShadow`'s own containment and fold rather than a second copy of the rule, `Punctual` finds a froxel's list through `FrameClusters.Of` and phase-weights `Lighting.Resolve` over it, and `Reproject` blends the previous volume ⚠ **in the volume's coordinates and not the screen's**, because a froxel is not a surface and a screen-space reprojection would find the pixel and know nothing about the slice. What is genuinely still absent is the punctual *shadow* inside that loop, which the shader's own comment marks: the punctual atlas is a composed feature of the scene pass and the fog passes are compute |
...
the BSDF library were correct, tested by nothing that shades, and callable from no material —
`DiffuseModels.OrenNayar`, `DiffuseModels.Burley`, `SpecularModels.Beckmann` and
`SpecularModels.MultiScatter` ([#1155](https://github.com/Rikarin/Vixen/issues/1155)). A layer written
and unreachable looks exactly like a layer that ships, from the shader library and from this table
alike.

⚠ **And "correct" was the wrong word for one of them, which is the sharper lesson.**
`MultiScatter` was reachable from nothing *and* wrong by a factor of up to half a million — at `f0 = 1`
and normal incidence its old body returned 170 at roughness 0 and 505 000 at 0.9 — so the audit that
...
| **TAA** | ✅ | `TemporalAntialiasingRenderer` in `Vixen.Rendering.PostFx`. It owns its history and alternates two textures, because a pass cannot read the target it writes — and they are *imports* rather than graph resources, since a transient dies at the end of the frame and a history that dies every frame is a history of nothing. The jitter is applied by `CameraExtractionSystem`, because what it offsets is the projection and that belongs to the view — for a long time it was only *exposed*, and nothing applied it, so every temporally resolved frame in the engine averaged the same sub-pixel sample over and over. Measured on sample 13's wall-top edge, applying it takes the staircase from an RMS of 0.277 px — the 1/√12 of whole-pixel quantisation — to 0.170. ⚠ Only when the tree has this node in it: a jittered camera nothing accumulates is strictly worse than a still one |
...
| SMAA | ✅ | `SmaaRenderer` over `Smaa.rvn` — three passes on one shader chosen by a `Mode` permutation, with the area table generated by `SmaaAreaTexture` rather than vendored, so no licensed data enters the tree. Reachable as `!Smaa` and as `AntialiasingMode.Smaa`/`TaaSmaa` on `!StandardFrame`. ⚠ Diagonal detection is deliberately not implemented |
| MSAA (**classic path only**) | 🟡 | 2/4/8× **colour** resolve is reachable from a document: `RenderTexture.sampleCount` and `RenderPass.sampleCount` already reached the texture and the pipeline, both backends already honoured `ColourAttachment.ResolveView`, and what was missing was any way to *name the pair* — `RenderPass.resolveTargets` is it, carried down through `RenderGraphPassBuilder.ColourAttachment`'s resolve argument. `MsaaResolveImageTests` is the first frame in the repository with any samples in it: 128 partly covered pixels on a 45° edge against the single-sampled draw's 0. ✅ **The depth resolve ships, and this row was wrong that it needs a shader.** The premise was right — averaging depth is meaningless, so it is a min or a max — but the conclusion did not follow: Vulkan resolves depth in fixed function through `VK_KHR_depth_stencil_resolve`, which this backend already enabled as a `VK_KHR_dynamic_rendering` prerequisite, and it takes exactly the min/max/sample-zero rule the premise identified. So depth resolves the way colour does — `DepthStencilAttachment.ResolveView` plus a `DepthResolveMode`, named on `RenderGraphPassBuilder.DepthAttachment` and reachable from a document by naming the depth target in `RenderPass.resolveTargets`. A shader was never an option here in any case: Raven has no multisampled texture type, `IrTextureDimension` is `{Texture2D, Texture3D, Cube}` and the SPIR-V emitter writes the `OpTypeImage` MS literal as a hardcoded 0, so `MSAADepthResolverShader`'s equivalent would have been a language change before it was a pipeline one. ⚠ **The mode is `Max` by default because the engine is reversed-Z** — near maps to depth 1, so the sample nearest the camera is the largest value, and getting it backwards renders plausibly while every consumer of the resolved depth reads a surface that is not there. ⚠ WebGPU has no depth resolve at all and the backend refuses one loudly rather than dropping it. ⚠ **And `!StandardFrame` deliberately offers no `samples:`.** Its expansion is the ambient split, which is not the classic path: making it multisampled means multisampled albedo, normals, specular and depth, and every one of those wants a resolve that is *not* an average — a normal is renormalised, an albedo is fine, a depth is a min. So MSAA is reachable from a hand-authored document and from a project's own forward frame, and asking the preset for it would be asking for four shaders that do not exist. ⚠ Not available on the virtualized-geometry path, and that is a decision rather than a gap: a visibility buffer breaks MSAA — per-sample visibility is four times the buffer and a per-sample resolve, which is the trap deferred falls into. TAA is shipped and owns its history, which is what Nanite leans on for the same reason. See [22](22-virtualized-geometry.md) § Phase 5 |
| Upscaling hook | P2 | a `IUpscaler` interface so FSR/XeSS/DLSS can be plugged; ship FSR1 (spatial, no licence friction) in-box |
| SSAO / GTAO | ✅ | `AmbientOcclusionRenderer` over `Ssao.rvn`, at half resolution by default — occlusion from a hemisphere is low frequency almost everywhere, so the cost halves twice and only contact edges notice. The march steps in the *depth buffer's* texel grid rather than its own half-size target's, which is the one thing about running it at a fraction that can be silently wrong. ⚠ **This row said "the full GTAO horizon integral is still to come" and it was right, against a shader whose own header claimed to be GTAO and an audit that had struck the line off as landed.** What ran was HBAO — the largest cosine of a sample's elevation above the tangent plane — which gives `1 − sin(e)/2` where the cosine-weighted visibility is `1 − sin²(e)/2`: equal at 0° and 90° and nowhere between. The arc integral (Jimenez et al. 2016 § 3.2) is now what runs, and what makes it worth the arithmetic is the normalisation rather than the accuracy at a horizon: an unoccluded surface reads exactly one at any tilt, which is a closed form `GtaoImageTests` holds it to. Bent normals are still a permutation nothing consumes, but they are at least a view-space direction now — its own row below, because a named absence inside a ✅ is how a leftover gets lost |
| SSR (screen-space reflections) | ✅ | `ReflectionRenderer` over `ReflectionTrace.rvn`, as `!Reflections` — a document names a depth, a normals plane and the colour a screen hit reflects, and gets the march. The hierarchical trace over the nearest chain is `PyramidMarch`, held sample for sample against `ScreenSpaceTrace`'s CPU reference. **Standalone is the default rather than a mode**: the field march's source defaults to `MaterialCompiler.EmptyFieldShader` and the miss to sky, so a project with no distance field and no surface cache gets screen reflections and nothing else. ⚠ That is not a reading of the defaults — `ReflectionRendererDeviceTests.TheNodeReconstructsMarchesAndPublishes` builds the node with *no composed sources whatsoever*, marches an imported depth-and-colour pair on a device, and holds the published plane against `TracedReflections` texel by texel. Standalone SSR is what that fixture has always been testing. `!StandardFrame`'s `reflections: Screen` emits it, and `!AmbientCombine` weights it in by `f0` |
| Bloom + lens flare | ✅ | `BloomRenderer`, in `Vixen.Rendering.PostFx`: Jimenez's 13-tap downsample and 9-tap tent upsample, one shader in three permuted modes. The pyramid is **declared**, so nine textures and nine passes vanish when nothing reads the result. Each pass steps in its *source's* texel grid — taking it from the target makes a bloom that is subtly too soft and that no screenshot answers. ⚠ **"Lens flare … still to come" is half stale and half owed, so light streak is its own row below.** `LensFlareRenderer` over `PostFx/LensFlare.rvn` landed, as `!LensFlare` and off `RenderQuality.LensFlare` |
| Depth of field | ✅ | `DepthOfFieldRenderer` over `PostFx/DepthOfField.rvn`, as `!DepthOfField` and off `RenderQuality.DepthOfField`. ⚠ **The row's "physical aperture params" is what shipped, and the manual mode deliberately did not**: every number comes off the view's `Camera`, because one aperture both brightens the image and shortens the depth of field, and a manual mode is what lets an author write a lens that cannot exist. A frame with no lens is a frame with no defocus rather than one with a default. ⚠ **Before the tonemap**, because defocus is scene-referred: a blurred highlight has to keep its energy |
| Motion blur | ✅ | `MotionBlurRenderer` over `PostFx/MotionBlur.rvn`, as `!MotionBlur` and off `RenderQuality.MotionBlur`/`MotionBlurSamples`, reconstructed from the motion-vector plane `MotionVectorRenderFeature` fills — so camera and per-object motion are one term rather than two paths |
| Tonemap + colour grading | ✅ | `TonemapRenderer`, with the 3D grading table bound and `UseLut` folding the sample out of the variant when there is none. ACES, AgX, Reinhard and Uncharted curves, exposure, white point, contrast, saturation, white balance and split toning are the shader's. ⚠ **This row's "what is still missing is the table as an asset" is retired**: `CubeLutImporter` (`Editor/Vixen.Editor.Assets/Textures`) reads a colourist's `.cube` and is registered in `BuiltInImporters`, which is the half that decides whether anyone can reach it — `Tonemap.rvn` had sampled a `Texture3D` with the correct half-texel inset since it was written, and a host had to build the texture in C# by hand, so nobody did. ⚠ **No mip chain, deliberately**: a mip of a colour transform is a different colour transform |
| Auto-exposure | ✅ | `AutoExposureRenderer` over `PostFx/AutoExposure.rvn`, as `!AutoExposure`, and `LocalExposureRenderer` beside it. ⚠ **It cannot be a full-screen pass like every other effect in the assembly**: its output is not an image but one number in a buffer, and a fragment stage cannot write a buffer at all — so it is a compute node plus a halving sequence of storage images. ⚠ **And the exposure buffer is a compositor *import*, not a declared resource**, which is the whole of what makes adaptation work: a declared buffer lives one frame, and one starting at zero every frame makes the ease-toward-target ease from nothing and never converge. `LocalExposure` is the second half a single exposure cannot do — a sunlit window and an unlit interior are twelve stops and a curve has six — and it is two passes because a fragment stage cannot read the texture it writes |
...
| Vignette, chromatic aberration, film grain, lens distortion | ✅ | `VignetteRenderer` (`Vixen.Rendering.PostFx/ScreenSpaceEffects.cs`): one pass, a permutation each, because they are one look and each is one or two taps. Grain moves with a frame index — grain that does not is a texture stuck to the screen, which is worse than none, and `LuminanceWeightedGrain` is what film actually does. ⚠ **Lens distortion is a fourth effect this row never named**, and it runs *first* in the shader because distortion moves where a pixel comes from: applied last it would warp the vignette and the grain along with the picture. Dithering was the fourth item here and is its own row below |
| Output dithering | ✅ **on every tier, and the seat is the lens node** | What stops banding in a gradient on an 8-bit display, and the cheapest thing in the whole post chain. `Vignette.rvn` declares `UseDither` with `ditherLevels` and `ditherIntensity`, applied last in the fragment — after the tonemap and after the grain, because it exists to break up the quantisation step of the final encode and dithering before a curve dithers the wrong quantity. The noise is the difference of two independent per-channel hashes, so it is triangular over ±1 code, and it is *static* rather than per-frame: the banding is spatial, and a moving pattern would add crawl to a still image to fix what a fixed pattern fixes. `DitherImageTests` holds it on a device against a ramp one code wide — the density has to track the ramp *and* the frame's mean must not move, because "the image changed" is satisfied by any noise including a bias ([#1156](https://github.com/Rikarin/Vixen/issues/1156)). The amplitude is one code of the *stored* value and not of the shader's: the lens node takes `VignetteAsset.Format`'s `Rgba8UNormSrgb` default, so `VignetteRenderer.Configure` sets the `SrgbTarget` permutation from the attachment's own format and the shader divides by the encode's local slope — a fixed 1/255 is about forty-five times too much noise in the shadows, which is exactly where banding is worst ([#1181](https://github.com/Rikarin/Vixen/issues/1181)). ⚠ **The lens node is therefore emitted on every tier, including the two with no vignette**, with all three of its looks permuted off — a tier-cost call taken deliberately rather than inherited ([#1243](https://github.com/Rikarin/Vixen/issues/1243)). Banding is a property of the *encode* and not a fidelity axis: an eight-bit target quantises a shallow gradient into visible steps whatever the cascade count is, and if anything the cheap tiers — no bloom, no volumetrics, nothing to break a gradient up — are the ones that band most. What Low and Medium pay for it is one full-screen pass that samples once, adds three hashes and writes |
| Light streak (anamorphic) | ✅ | ⚠ **Split out of the bloom row, whose "lens flare and light streak still to come" was half stale and half owed.** `LightStreakRenderer` over `PostFx/LightStreak.rvn`, as `!LightStreak`, off `post.lightStreak` and emitted by `!StandardFrame` after the flare and before the bloom at the Epic tier. A threshold, then `blurPasses` separable blurs along one axis whose stride grows by `2·samples + 1` each time — the width the pass before it produced, so the intervals tile with no gap and three passes of four taps reach 729 texels for 27 samples. ⚠ **Each pass steps in its *source's* texel grid** — the trap this row's bloom sibling records having fallen into, and the reason the reduced plane's extent rather than the scale is what the texel is derived from. `LightStreakImageTests` holds the shape on a device with the closed form [#1159](https://github.com/Rikarin/Vixen/issues/1159) asked for: one bright texel must give a run along one row whose length is a function of the pass count, and *exactly* the scene off that row — which an isotropic blur fails at every radius, and a smear of any width would pass an eyeball comparison |
| Bent normals consumed by the ambient term | ✅ | ⚠ **Split out of the SSAO/GTAO row, which recorded it inside a ✅ as "a permutation nothing consumes"** — doc 43's `--scale` shape in the renderer, an output nothing reads, which no gate can tell apart from an output nothing produces because both are green. Both halves exist now: `AmbientCombine.rvn`'s `useBentNormal` evaluates the environment along the direction instead of the geometric normal ([#1157](https://github.com/Rikarin/Vixen/issues/1157)), and `!StandardFrame` turns the producer and the consumer on together from one tier column, `gi.ssaoBentNormal` — off at Low and Medium, on at High and Epic ([#1195](https://github.com/Rikarin/Vixen/issues/1195)). ⚠ **One column and not two switches**: the permutation moves the occlusion out of `r` into `a` to make room for the direction, so a frame with only the producer on multiplies its ambient by a direction's x — a number in [0, 1] that reads exactly like an occlusion |
...
### 🟡 Status: both targets run on a device; the *draw* is still the CPU's

`Vixen.Vfx` has the storage, the compiled graph, the CPU simulation, billboard geometry and
`ParticleRenderFeature`. `VfxShaderEmitter` closes the other half of the dual target: the same
...
⚠ **This section read "what is not built is the dispatch" long after the dispatch was built.**
`Core/Vixen.Rendering/VfxGpuSimulation.cs` owns the buffers an emitted shader binds, the descriptor
set that names them, the shared pipeline layout, the two kernels' dispatches, the upload and readback
paths, GPU reaping behind an `atomicAdd`, and a `DrawIndexedIndirect` command it keeps up to date;
`VfxGpuSort.cs` is the bitonic sort beside it. `VfxGpuDispatchTests` and `Platform/Vixen.Vfx.Gpu.Tests`
run all of it on a device, and the exit criterion's CPU/GPU agreement is met twice over — as numbers
in `VfxAgreementTests` and as two pictures in `VfxBackendPictureTests`.

⚠ **What is actually owed is the draw, and it is a narrower gap than the one this section named.**
The simulation's buffers are created `Storage | CopySource | CopyDestination` with no
`BufferUsage.Vertex`, no shader in `Raven/Library/Vfx` reads a particle out of a buffer in a vertex
stage, and nothing in the tree draws through `VfxGpuSimulation.DrawArguments` — so a GPU-simulated
effect reaches the screen only by being read back and expanded by the CPU path, which is what
`VfxBackendPictureTests` photographs and says so in its own header. **The remaining work is a vertex
path, not a compute one.**
