...
| Output dithering | ⬜ | ⚠ **Split out of the row above, where it was hidden inside a ✅.** What stops banding in a gradient on an 8-bit display, and the cheapest thing in the whole post chain — a triangular-PDF noise added before the encode. Nothing in the tree spells it: `Vignette.rvn` has five permutations and none is dither. `VignetteRenderer` is the natural home, since it is already the pass that runs last and already carries a frame index for the grain. [#1156](https://github.com/Rikarin/Vixen/issues/1156) |
| Light streak (anamorphic) | ⬜ | ⚠ **Split out of the bloom row, whose "lens flare and light streak still to come" was half stale and half owed.** `LensFlareRenderer` landed; nothing in the tree spells a light streak — the word appears only in `TonemapRenderer`'s prose and in `VfxRenderer.Streak`, which is a particle shape and unrelated. `FullScreenRenderer`'s additive blend is what such a pass would want and it exists. [#1159](https://github.com/Rikarin/Vixen/issues/1159) |
