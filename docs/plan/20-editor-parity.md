...
| **Project browser** | Content Browser / Project | `.App` | 🟡 | The folder tree, saved filters and **collections** are all built — [#1040](https://github.com/Rikarin/Vixen/issues/1040) is closed — and the source-control column is built over `ISourceControl` and draws git's own letters ([#1034](https://github.com/Rikarin/Vixen/issues/1034)) — ⚠ on the **grid**, which is what the panel opens as; the list view has no cell to put one in, because `TreeNode` has no trailing accessory at all ([#1171](https://github.com/Rikarin/Vixen/issues/1171)) — which is the same gap the outliner's per-entity visibility and lock waits on. ⚠ The tree and the grid are two *modes* of one view, so the folder tree is a second, folders-only control that narrows the grid rather than the existing one turned on as well. ⚠ A saved filter keeps the **query** and a collection keeps the **result** — one `AssetId` set, because paths break on a move — which is why only the first of the two is a preference: `EditorPreferences` is user-wide and an `AssetId` means nothing in another project, so the collections are a settings asset under `ProjectSettings/`. ⚠ **Not `Library/` either**, which is the other per-project home: everything there is reproducible from a source file plus its `.meta`, and a collection is authored data nothing can rebuild. They live in the folder column under a **Collections** heading — a drop on the heading makes one, a drop on a row adds to it, and choosing one narrows the grid the way a folder does |
...
| **Layers and tags** | Layers / Tags & Layers | ⛔ | Needs an ECS-side concept first. On the Scene menu, disabled with that reason. ⚠ **The block is the work**, and it is a decision rather than a panel: what a layer is, where it lives on an entity, and which of the three consumers that each want a different shape — rendering masks, physics matrices, the outliner's filter — it is designed for. ⚠ **But "nobody has decided" understates the tree**: `Vixen.Physics/PhysicsLayers.cs` already *is* the whole shape — a `PhysicsLayer` byte index, a one-word `PhysicsLayerMask`, a 32-entry name table, `TryFind(name)`, and a symmetric collision matrix — and it is the one of the three consumers that is real. Of the other two, the per-view mask does not exist (`RenderView.Stages` is a `RenderStageMask`, a different axis) and `SceneHierarchyView` has no filter at all, so both are wishes rather than callers. So the decision is *promote, federate, or leave alone*, not *invent*. [#1036](https://github.com/Rikarin/Vixen/issues/1036) |
...
| **Physics debug** | — | ⛔ | Collider draw, contact visualisation, layer matrix. ⚠ **The same panel doc 13 owes as its ninth overlay, and it must not be built twice** — the editor's is the authoring side of one mechanism, and the ingredients are in place either way: `DebugDraw` batches wire boxes, spheres, capsules and frustums, and the editor has a `PhysicsScene` over the world being edited. [#346](https://github.com/Rikarin/Vixen/issues/346) |
| **Terrain / foliage** | Landscape + Foliage / Terrain | ⛔ | ⚠ No longer Post-1.0 — [Part G](#part-g--out-of-scope)'s row was reopened. Two modes rather than one, because sculpt and paint need a terrain and foliage paints onto any surface; the panel is a create/manage section with a derived cost readout, the edit-layer stack, the target layers, and one brush shared by every tool in both modes. [31 § Part 2](31-terrain-grass-and-trees.md#part-2--the-authoring-surface) |
...
| **Source control** | Revision Control / Version Control | ✅ | The seam, a git provider, the column, Revert, and the Revisions panel — an asset's history with the picked commit's diff under it and Restore This Version beside it. `ISourceControl` is four methods — a sweep of the working tree, a revert, a per-asset log and a per-revision diff — with `GitSourceControl` over the client the user already has, and both browsing surfaces draw a letter — a corner on a tile, a trailing mark on a list row. ⚠ **The list view was believed to want a new part on `TreeRow` and it did not** ([#1171](https://github.com/Rikarin/Vixen/issues/1171)): a row is a control with children and `tree-label` grows, so anything added after it lands at the trailing edge — which is what the outliner's eye and padlock have done since Part D, and what `TreeView.RowBound` exists for. It was an editor wiring gap and not a control gap. ⚠ **The sweep is the design decision and the row's four pieces hid it**: a provider asked per file launches a process per tile and one asked on a timer answers from a cache an external checkout invalidates, so the status is taken on the two events that already mean the project moved underneath the editor — the watcher draining and Refresh — and once when the panel opens. ⚠ **A folder takes the loudest status under it**, or the column only helps in the folder you are already standing in. ⚠ **`--porcelain -z` and `rev-parse --show-prefix`, neither of them decoration**: the default output quotes a name with a space in it, and porcelain paths are relative to the *repository* while every path in the editor is relative to the project — a project one directory down would have had a blank column for every file. ⚠ **Diff and history turned out to be one panel rather than two verbs** ([#1170](https://github.com/Rikarin/Vixen/issues/1170)): a history row's value is what a person does with it — see what that commit changed, or take that version back — so a list with nothing under it is a list of sentences, and a diff of the version you already have answers nothing. There is deliberately no `assets.diff` command. ⚠ **And the design question the issue said had to be answered before a viewer — what a *binary* asset's diff says — is answered by refusing to invent one**: `SourceControlDiff` carries an `IsText` flag, and a `.png` or a mesh gets git's own `--stat` sentence (`Bin 1234 -> 5678 bytes`) rather than a patch with the bytes elided. ⚠ **`--follow` is why this is worth asking per asset at all**: without it the log stops at the first rename, and every asset is renamed the first time somebody tidies a folder. What is still owed is a two-column viewer with intra-line highlighting — the panel draws a single-column unified patch. [#1034](https://github.com/Rikarin/Vixen/issues/1034) |
...
|---|---|
| **The picking stage driven by a real target** | ⚠ **The reason stated in [B2](#b2--the-viewport) was wrong in a way worth recording.** It is true that `EditorHost` owns a `RenderGraph` and that `ScenePresenter.Declare` hands back a `GraphTexture` — and neither is what `PickingRenderer` consumes. It is a `SceneRenderer` over a `RenderStage`, so it needs a `GraphicsCompositor` and a render system feeding it, which the editor's viewport does not have: what draws the scene is `SceneMeshes` through `MeshInstanceRenderer`, which has device-resident geometry and a per-entity transform — [blockout-tools § B1](24-blockout-tools.md#b1-every-mesh-in-the-viewport-went-through-the-cpu-every-frame-) — and no compositor and no materials. Clicking and banding both work, exactly, through the processor — and that is the right answer for primitives and the wrong one the day a shader moves a vertex |
...
| **Scenes-in-build is a list of scenes** | And what reads it is the content build and then the player. The build resolves every entry to the address its sidecar declares and writes them, in order, as a `SceneManifest` beside the catalog — refusing an entry that names nothing or names a scene with no address, because either one ships a player that starts to an empty world. `AppConfig.StartupScene` defaults to the first of them, so the order means what the panel says it does. ⚠ **`PlayerBuildSettings` moved to `Vixen.Editor.Core` to make that true for both heads**: the build that reads the list is `ContentPipeline`, which `vixen content build` runs too, and a settings type only the editor could see would have meant the editor's player opening a level and CI's opening nothing |
...
`PlayerBuildSettings.Target` — in `Vixen.Editor.Core`, so the CLI's build reads the same file — is
what ships. One field would be simpler and would make "build the
...
| **Panel lifecycle** ✅ | Every registered panel is built, docked, floated, closed and rebuilt in one test. `A panel's factory runs again when it is reopened` is already a documented hazard and nothing else proves a given panel survives it. Closed-and-reopened is `MilestoneE3Tests` and found a real defect on the first run and another when four more panels were registered; floated-and-brought-home is `PanelFloatLifecycleTests`. ⚠ **This row used to say floating could not be covered "because tearing a panel out needs a window host and the harness has none", and that refusal had expired**: `IUiWindowHost` is four members in `Vixen.Ui` with no platform in them, and `Vixen.Ui.Controls.Advanced.Tests` had been driving the whole tear-out path over a double of it for as long as the path had existed. What was missing was the double on the editor's side, not a display server |
...
| **Terrain and foliage tools** | A whole subsystem — heightfields, layers, sculpting, procedural scatter, LOD, and a renderer for each — behind a mode. Post-1.0, and the `IEditorMode` seam in [A1](#a1--the-application-frame) is what it will attach to. ⚠ **Reopened.** Every clause above is still accurate, including the size — what changed is the denominator. When this was written the mode seam had one implementation, the viewport gathered meshes through the CPU every frame, and there was no GPU culling, no bindless table, no instancing feature, no page residency and no `SampleLevel` in a vertex stage. All of those now exist and were built for other reasons. [terrain-grass-and-trees.md](31-terrain-grass-and-trees.md) is the plan, and the table in it is where the line now sits |
