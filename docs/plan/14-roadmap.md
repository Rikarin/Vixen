...
| [28](28-gameplay-framework.md) | The gameplay library set on top of it | § Cost. 25.5 EM across G0–G8, taken by genre rather than whole. Its libraries live in `Gameplay/`, a top level whose build plumbing landed with 27's L0. **G0 — the kernel — has landed**: tags, `DefId`, definitions and their catalog, the attribute algebra, effects, requirements, the RNG, the module seam and the `.vxdef` importer. It is the one milestone that document says is not optional, and everything above it is definitions and rules over those types |
