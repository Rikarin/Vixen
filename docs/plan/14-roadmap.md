...
| [28](28-gameplay-framework.md) | The gameplay library set on top of it | § Cost. 25.5 EM across G0–G8, taken by genre rather than whole. Its libraries live in `Gameplay/`, a top level whose build plumbing landed with 27's L0. **G0 — the kernel — G1 — items, the container algebra and loot — and G2's combat half have landed**: tags, `DefId`, definitions and their catalog, the attribute algebra, effects, requirements, the RNG, the module seam, the `.vxdef` importer, a sixteen-byte item instance with affixes regenerated from its seed, transactional containers with the conservation oracle in CI, loot tables with durable pity, a drop simulator that runs the shipped evaluator, and abilities over a six-stage damage pipeline with threat and taunt. G0 is the one milestone that document says is not optional; G2–G8 are independent tracks a game takes by genre |
