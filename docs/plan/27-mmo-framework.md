...
| `IGuildGrain` ✅ | guild id | roster, ranks, bank, guild hall shard. ⚠ **The grain decides ordering; the caller decides permission.** A charter's permissions are tags on compiled content, so *"may this officer kick"* is the realm's question and it answers it with the same code the client greys the button with. What no local check can win is the *race* — two officers demoting each other at once — so the grain re-checks the one part that is arithmetic: rank is an integer and you may not act on somebody at or above your own. That is `HousePlot`'s split a tier up. The bank is a `LedgerAccount`, not a field, so a deposit is an ordinary two-legged transfer and the conservation oracle covers it. ⚠ **#39's bridge found the constraint this row implies and does not state: a realm can only name the members who are connected to it.** A gameplay `PlayerId` is a session id widened, so a 500-member guild seats maybe thirty — which means **the write has to be an operation and can never be a roster**, or it deletes everybody who was offline. It also means every method here needing a `by` is load-bearing in a second way: a diff of two rosters cannot say who moved anybody, so a state-shaped save is unimplementable against this interface *even if* the roster were whole. And **kicking an offline member is a service-plane action**, not a realm one — there is no realm that can express it. ⚠ One gap left open on purpose: there is no `Rename` for the guild's *own* name, only `RenameRank`, so a realm cannot write one and the bridge does not diff `Guild.Name`. Renaming a guild is a service-plane action for the same reason a kick of an absent member is |
| `IQueueGrain` ✅ | queue id | matchmaking tickets and pools ([28](28-gameplay-framework.md) § Matchmaking). ⚠ **The scoring stays a pure function and the grain is a scheduling decision on top**, which is `PlacementDirector`'s relationship to `IMapGrain` — a matchmaker only exercisable inside a silo is one nobody tests. ⚠ **Formed is not started**: a roster still needs a shard and allocating one can fail, so the tickets are held and the caller confirms or abandons — L2's reservation at a different scale. ⚠ **Abandoning keeps the original enqueue time**, or a ticket is punished for a failure that was the fleet's. ⚠ **Backfill is preferred to a new match**, because a running game with an empty seat is worse than one that has not started |
| `IInstanceGrain` ✅ | instance id | a dungeon/raid/story instance: access list, lockout, lifetime. ⚠ **A lockout is fleet-wide, which is why it is a grain and not a realm's table** — doc 28: *"a lockout one shard knew about is a lockout a player evades by zoning"*. ⚠ **Progress belongs to the instance and not to each player**, so somebody bound late inherits what is already dead; per-player progress is a raid re-killing its first boss for every latecomer. ⚠ **Binding cannot be undone and there is no method for it** — that is what a lockout *is* — and **closing releases nobody**, or disbanding is how a group runs a raid twice |
...
> **Built, and it found two leaks.** Every budget above holds — zero conservation violations over 142 005 transfers, zero disconnections through the rollout, version spread zero, tick p99 76 µs — and thirty minutes of fleet takes ten seconds of wall clock.
>
> ⚠ **The oracle's obvious implementation is wrong, and this document should say so.** Summing every account of the ledger gives zero by construction: a journal is double-entry, so every movement has two legs — and **a duplicate is two balanced legs**. Conservation means the sum over the *player* accounts is what was minted. It also has to be checked **every tick** (a duplicate that is later spent is invisible to a final total) and it has to **count the players in transit** (they have been released by one shard and not yet admitted by the other, so counting only the shards shows a dip on every transfer that corrects itself — which is what a duplication bug looks like from the other side).
>
> ⚠ **A rollout is not a thing that happens in one tick.** The first version drained all eight shards in a single step. It passed both assertions and it was not a rollout: the interesting state is the middle, where half the fleet is on the old build and players are still travelling across the seam — and moving two hundred people at once is the admission spike a drain exists to avoid. Four a tick, emptiest first, and a shard that will not empty is waited on.
>
> ⚠ **A realm that drains its outbox and forgets to settle leaks without bound**, and looks fine for the first ten minutes. That is `LedgerBridge`'s documented behaviour — *"a drained write is not removed"* — working exactly as written, and `Pending` is the number a fleet should alarm on. § Diagnostics should carry it.
>
> ⚠ **The idempotency-key set grows for ever.** 199 379 keys after thirty minutes, and a shard that runs for a week keeps every key of that week. It cannot be cleared — it is what makes § Persistence's second rule true — so it needs a **horizon**, and how long that horizon is is safety-critical rather than a tuning knob: a retry arriving after it is no longer recognised as a replay and is applied again. That number belongs in this document rather than in the class.
>
> ⚠ **The horizon's argument is the retry window, not the horizon, and this document is where the window is stated.** The two failure modes are not comparable — too long is memory, visible and recoverable; too short is a duplicated item, invisible and permanent — so `KeyHorizon.Outliving` is the only bounded constructor and a horizon shorter than the retries it must outlive is unrepresentable. There is **no default**: a number nobody chose is the same risk with the evidence removed, which is why the leak is what a ledger has until a game says otherwise. § Diagnostics should carry `Keys` and `Forgotten` together, because a flat key count with nothing ageing out is a realm that is not sweeping and looks healthy.
>
> ⚠ **A horizon alone is not enough, and the outbox is what closes the gap.** A key can age out while the write carrying it is still unsettled, and then the realm applies the movements a second time — on a realm whose *database* is right, because the durable key refuses the duplicate write. So the bridge asks the outbox before the projection: it is exact for everything started here and not finished, which shrinks what the horizon has to cover to *retries after the write is durable*.
>
> ⚠ **And the key set was not the whole of the leak — it was 45 MB of 168.** Two more, both of the same shape and both only findable by ablation. **Every departed player's balance rows**, because `Restore(…, 0)` reads as the mirror of admission and is a no-op; the real mirror moves the rows to the world account they were seeded out of and drops them, and is deliberately *not* an intent, because a handover is not a movement of value. **And every departed player's social graph** — 130 MB of the 168 — because `SocialGraphs.Of` mints on demand and nothing took one away. ⚠ Dropping the departed player's own graph is half the job: a gameplay id is never issued twice, so a friend still online keeps an id no re-admission will replace, and only the durable set knows who held a tie to whom. **The general lesson is the ordering one: a realm is a pile of caches keyed by a realm-scoped id, and every one of them needs a mirror of its admission path.**
