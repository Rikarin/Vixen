...
- **Measured when the development flavour first met a real Homebrew install** (2026-07-26, Apple
  ...
  | `vkCreateInstance` → `ERROR_LAYER_NOT_PRESENT` for a layer `vkEnumerateInstanceLayerProperties` had just listed | Homebrew's layer manifest names its library by bare filename, which the Loader resolves through `dlopen` — and that has the same search path | the build exports `DYLD_LIBRARY_PATH` onto the process that starts the test assemblies (`Build.ExportLayerLibraryPath`; `.runsettings` did this until [#560](https://github.com/Rikarin/Vixen/issues/560) moved the runs off VSTest, and still does it for `Coverage`); the backend also retries without the layer and logs event 2002 rather than refusing to start |
