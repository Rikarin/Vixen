...
**5. `style()` queries** — not started, not costed, and not needed by the editor's case. It is a
different shape from the size half: `ContainerQuery` evaluates a box, and a style query evaluates a
custom property's computed value on the container.

**Size: 0.15 EM remaining**, from 0.4. The variants, the marker family and the units landed; the
coercion and `style()` are what is left.
