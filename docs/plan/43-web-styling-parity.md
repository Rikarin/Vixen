...
| Utility **roots** (the unit of this table) | **331** | 322 families |
...
| State | Meaning | Roots |
|---|--:|--:|
| **works** | Vixen emits it, and a consumer acts on every property it sets | **260** |
...
⚠ **And the variants, which this ledger has no row for and cannot.** Every row is a utility root and
every measured column comes from a property something emits; a variant emits none, so the consumption
gate never sees one. What stood in for a measurement was prose — A12's row saying its pseudo-element
variants wait behind the generated box, carried through six audits and never checked. It is **16 of
v4's 88**, in `docs/plan/43-web-styling-variants-unsupported.txt`, answered by
`Variants.TryResolve` over a probe class because a variant is a prefix rather than a class. ⚠ The
probe is chosen from forms **v4 itself accepts**, bare first and arbitrary last: `group-[3]` compiles
in v4 and Vixen refuses every arbitrary `group-`, so an arbitrary-first probe libelled two variants
Vixen has.
...
✅ **The coercion landed (#273), and it needed no new layout path.** ⚠ Two claims above were wrong by
the time it was attempted. "Coercion cannot be expressed from outside `Vixen.Ui.Layout`" and "no
single choke point" both predate `contain`: `LayoutTree.CalculateLayoutImpl` pins a size-contained
axis before any algorithm runs — `MeasureNodeWithoutChildren`, then `StretchFit` at that size — which
is exactly the coercion, for every path listed above at once. `container-type` simply never reached
it: `UiDocument.KindOf` read the declaration for the query and `LayoutStyleBuilder` never read it at
all. `ContainmentReader` now folds `container-type: inline-size`/`size` (and the `container`
shorthand's type, longhand winning as `KindOf` has it) into inline-size or size containment, which is
CSS Containment 3 § 3.1's own reading rather than an engine rule. ⚠ **Plus an independent formatting
context and *not* layout containment**: § 3.1 used to say layout containment and was changed, and the
observable difference is the containing block of an absolute descendant, so it is a new
`Containment.FormattingContext` flag that `EstablishesBlockFormattingContext` reads and
`LayoutTree.Absolute` does not. The seesaw fixture above now settles with the container nought wide
and the body 900; `7007` survives for the loop containment cannot close — a verdict that moves the
container onto another flex line — and its fixture is now that. `ContainerTypeContainmentTests`
...
`@max-*` emits the exclusive form. ✅ **And v4's unit, since #1373**: both evaluators read `rem` and
`em`, and the two at-rules measure them against different fonts. `@media` uses the initial font size
(Media Queries 4 § 1.3), which is `MediaContext.FontSize`, the document's `RootFontSize`, so
`min-[40rem]:` is 640 pixels at 100 % and 800 at 125 %. `@container` uses the container's computed
...
inclusive, which is v4's reading too and the one `Screens` gives every breakpoint. ⚠ **#609 closed on
the container half only**: its own body said the media half "should land with it or be refused with
it", and `max-sm:`/`min-sm:` were not classes at all until #1346 — `Variants.TryScreen` now reads the
same `max-`/`min-` prefixes and arbitrary forms, `max-*` emitting `(width < …)`, asserted at the
threshold. ⚠ Landing it exposed that the generator wrote its at-rule groups in **ordinal** order, so
`@media (min-width: 1024px)` came before `(min-width: 640px)` and `sm:p-2 lg:p-4` drew `p-2` on a
wide window; `AtRuleOrder` now writes them in v4's order (viewport `max-*` widest first, then `min-*`
narrowest first, then containers the same way, then everything else). ⚠ Discrete
...
**3. The `@container` *marker* family ✅ landed**, and it needed two things the variants did not.
`@container` is `container-type: inline-size`, `@container-normal`/`-size` the two keywords,
`@container/main` a `container-name` beside the type, and `@container-[…]` the arbitrary form.

⚠ **The scanner was the first blocker and it was not predicted.** `CandidateScanner` refuses every
colon-less `@` run, by grammar rather than by a list of names — an at-keyword never has a colon and a
container *variant* always does — and this class is an at-keyword letter for letter. So it admits
exactly this one name, followed by nothing, a hyphen or a slash, and refuses every other. The cost is
the scanner's usual one: a sheet writing `@container (min-width: …)` yields the candidate and one
unused rule is generated.

**The probe scene was the second**, as predicted: `container-type` moves none of the four channels
unless the scene holds a query reacting to it. `queried` is a typed probe inside a typed host with an
unnamed and a named query waiting on it, and the three values are separable — injecting `normal`
*stops* the probe being a container, so `.kid`'s query falls to the wider host and the layout moves.
⚠ **`container-name` needed a third thing, and it is the one the surface had never had to spell**:
it is emitted only through a modifier, and `UtilityFamilies.Surface` enumerates keywords and values
and no modifier at all — so without a slash probe the property would have been in neither the read
set nor the inert one, unclassified, which the probe's own remarks call worse than wrong. Adding it
turned up that `ParityLedger` and `UtilityFamilySupportTests` both read a surface entry with
`SplitName`, whose contract is a class whose modifiers are already off; both parse now.

**4. `cqw`/`cqi`/`cqb` units** ✅ — all six of CSS Containment 3 § 5.3's units (`cqw`, `cqh`, `cqi`,
`cqb`, `cqmin`, `cqmax`) parse, resolve against the query container's content box and reach the
layout tree, with `ContainerUnitTests` measuring each as a width or a height on an element inside a
container of a known size. ⚠ **`cqi` and `cqw` answer the same number and that is exact rather than
approximate**, as `inset-x-*`'s row three sections up records for the same reason: the inline axis is
the horizontal one in every writing mode `Vixen.Ui.Layout` has, and it has one.

⚠ **Three things about it were not obvious, and two of them were nearly shipped wrong.**

- **The units are read off the style walk, not off `ContainerScopes`.** A `cqw` needs no
  `@container` rule — `container-type: inline-size` on a panel and `width: 50cqi` on its child is a
  complete stylesheet — and `Recontain` gives up before entering a single scope for a document that
  declares no container group. Reading the chain would have resolved every such document's container
  units against the viewport, silently.
- **The two axes can come from two different ancestors.** An `inline-size` container cannot answer
  `cqb`, so the nearest `size` container above it does and may be absent, in which case the viewport
  does. One box carried down the walk gets this wrong by a plausible number.
- ⚠ **It needed the settle loop, and needed to not cost one.** Styles are built before layout runs,
  so the first pass resolves every `cqw` against an unmeasured container — and `Apply`'s reference
  test on `ComputedStyle` would then declare the element up to date for ever, the `lh` trap one unit
  along. The container's two sizes are in that test now. The extra pass is asked for only when
  `Recontain` is not going to ask, because it invalidates already whenever a container's box moves —
  and an unconditional invalidate cost every document with a query container a settle pass it did not
  need, which `ContainerWiringTests` measures to the pass and caught.

**5. `style()` queries** — ✅ **the unnamed, style-only form landed (#273)**, then the named and
the mixed forms (below). It is a different shape from the size half: `ContainerQuery` evaluates a box, a style query evaluates a custom property's computed value, and CSS Conditional 5
makes every element a style container — so `@container style(--variant: primary)` asks the
*parent*, whose resolved style the cascade already holds when it resolves the child. It is answered
there (`StyleQuery`, `ContainerConditions.StyleHolds`, one test in `StyleResolver.Cascade`), never in
`ContainerScopes`, whose chains are boxes. ⚠ **ExCSS 4.3.2 does not parse `style()`**: it hands the
prelude over as the `ConditionText` "not all", so the loader reads the raw span instead — and "'not
all' is not a container feature" was what every style query in a sheet used to produce. The unnamed
form has exactly inheritance's invalidation, which `ContainerWiringTests` exercises through a class
toggled twice on a live document.

✅ **The named form landed too (#273), with the two things it was refused for.** `@container card
style(…)` asks the nearest ancestor whose `container-name` list includes `card`. That can be any
element, because every element is a style container. The ancestor's style is reachable through
`StyleResolver.ResolvedAncestor`, which `StyleUpdater` and `StyleEngine.ResolveAll` point at their
own arrays. The resolver cascades the chain itself when neither is running. The invalidation edge is
in `StyleUpdater.Drain`: when a sheet declares a named style query, a named element whose style moved
re-resolves its *whole* subtree. The ordinary rule stops at an element whose inherited portion did
not move, and an element between the card and the asker that declares the same property is exactly
that. `ContainerWiringTests.A_named_style_query_follows_the_named_ancestor_past_an_override` reads the
stale answer when the edge is removed. `or` and a single `not` over `style()` features landed with it.
`not` over no container matches nothing, because CSS treats that query as unknown and unknown is not
negated into a match. `and` mixed with `or` needs parentheses in CSS, and a parenthesised group is
refused.

✅ **The mixed form landed too (#273)**, as two groups rather than one. `(min-width: …) and
...
across the two halves is not nested: its size group is registered beside the style group, and
`ContainerConditions.StyleHolds` reads that group's verdict as a disjunct. ⚠ **That form was
refused for a reason that was wrong** — "no single place to be answered". CSS Containment 3 § 5.1
gives it the same one element the `and` form asks, and the cascade holds both that element's style
and its box's verdict. The two halves pick the same element because both apply one
...
`StyleUpdater`'s whole-subtree edge covers every size container once a sheet declares a mixed query,
because the unnamed mixed form asks one that need not have a name.
`ContainerStyleQueryPixelTests.A_mixed_query_colours_the_label_only_when_its_size_container_passes_both_halves`
moves each half in turn over three counted frames.

✅ **The size half reads `or` and `not` too (#273).** `ContainerQuery` split on the literal
`" and "`, and ExCSS hands an `or` prelude over as "not all", so every `or` size query was refused.
⚠ **And `@container not (min-width: …)` loaded with no diagnostic as a query for a container
*named* `not`**, because ExCSS reads the first word as the name, and it never matched. The loader
re-reads the source text where ExCSS lost it or read a reserved word as the name.

Still refused, deliberately: `style()` on a standard property, which no shipping engine answers
either and which would need a computed-value comparison per property, and parenthesised groups,
in the size half as in the style half.

**Size: done, from 0.4.** The variants, the marker family, the units, the coercion and the unnamed,
named and mixed `style()` forms landed. What is refused is refused with a reason.
...
| A12 🟡 | Pseudo-elements materialised — `::before`/`::after` with `content`. ⚠ **`placeholder:` and `placeholder-*` are landed, and neither needed A12**: `::placeholder` names a box `TextField` already builds as `Part("field-placeholder")`, so the variant is a child combinator in a table of its own (`Variants.Parts`) and the colour root is a child-scoped family on the same scope — Part 10 Bucket 2 has the correction. The generated box itself is not started, and four passes agree on why: the smallest slice anything calls is a second style slot for a box that is not one of the container's children, across `StyleRuleSet`, `UiDocument`, `Vixen.Ui.Layout` and the draw list, and every smaller one is a named failure mode — registering the seven variants first emits selectors `SelectorCompiler` refuses, which is F6's own defect. ⚠ **The lead the third pass left — that `::placeholder` and `::selection` name something that already exists and could land without the generated box — is half right, and the two halves are different problems.** `::placeholder` IS an element: `TextField` builds it as `Part("field-placeholder")`, a direct child with its own tag that `ControlTheme.vcss` already styles. But it cannot be a row in `Variants.States`, because `not-`, `has-`, `group-` and `peer-` all compose over that table — a descendant-shaped suffix makes `not-placeholder:` mean "an element that is not a placeholder" and `group-placeholder:` mean nothing at all, each of them valid CSS meaning something else, which is F6's failure mode one level up. `::selection` is not a box at all: `TextField` paints the highlight from `--selection-color` read off its own style, so `selection:bg-*` has to rewrite the utility's PROPERTY. ✅ **That fourth variant shape landed (#233)**: `VariantEffect.Rewrite` and a `Variants.Rewrites` table, so `selection:bg-*` emits `--selection-color` on the element — inherited, which reaches the element and its descendants as v4's `& *::selection, &::selection` does — and a utility with any other property (`selection:text-white`) is refused rather than emitted unmoved, because nothing paints selected glyphs a colour of their own. ✅ ⚠ **The debt is measured now rather than described**: `Tools/Vixen.TailwindParity` hands every one of v4's 88 variants a probe class to `Variants.TryResolve`, and the 16 it refuses are enumerated in `docs/plan/43-web-styling-variants-unsupported.txt` — six of them pseudo-elements. ✅ **`backdrop:` landed without the box (#233), and the reason it was refused was false**: the variant table said `file` and `backdrop` "name controls that do not exist", but `Dialog` and `Drawer` build the sheet behind them as `Part("dialog-backdrop")` and `Part("drawer-backdrop")`. So it is a second `Variants.Parts` entry, a child combinator onto `:is()` of the two tags, asserted on a real opened `Dialog` and `Drawer` over the loaded control theme down to the draw list's one magenta rectangle. ✅ **`details-content:` landed the same way (#233), and it too was listed behind the box**: `Expander` is this framework's `<details>`, `ExpanderHeader` its summary and `Part("expander-content")` the slot `::details-content` names, so it is a third `Variants.Parts` entry, `> expander-content`, asserted on a real `Expander` whose frame holds no magenta while it is closed and exactly the slot's rectangle once it opens. It is the `expires-on` idea in the one form that cannot be walked past: the day `before:` resolves, TWP009 fails and says to delete the line | `StyleRuleSet`, `UiDocument` | — | 0.5 |
...
| A16 🟡 | Container queries. ⚠ **The cascade half landed** — `ContainerConditions`, `ContainerScopes`, `ContainerQuery`, a second group id on `StyleRule`, two scope slots on `StyleTree`, one integer test in the cascade, 34 computed-value tests. ⚠ **And it closed a silent drop**: ExCSS parses `@container` into a `ContainerRule`, so it never reached `LoadUnknown` and was discarded with no diagnostic, while two docs said it warned. ⚠ **And the `@sm:` variants were never gated on the wiring**: a pure variant emits no property, so the consumption gate never sees one — the blocker was that `--container-*` did not exist, and `Screens` under the same names means numbers two-thirds too big. They are registered now, off `ThemeTokens.Containers`. ⚠ **And the marker family landed, blocked on the *scanner* rather than on the cascade**: `@container` has no colon, so the rule that keeps `@apply` and `@media` out of the candidate set kept the class out with them — the scanner names this one class and refuses every other at-word. Its probe scene is `queried`, and `container-name` wanted a slash probe on `Surface`, which had never spelled a modifier and left the property unclassified without one. ✅ **The layout coercion landed** (#273): `container-type` applies inline-size or size containment and an independent formatting context through `ContainmentReader`, reusing the containment branch `contain` already had, so a container sized by its contents takes no size from them and settles; 7007 now reports only a container its surroundings move. ✅ **Unnamed `style()` queries landed** (#273), answered in the cascade against the parent's resolved style; ExCSS turned every `style()` prelude into "not all", so the loader reads the raw span. The named form landed after it, with the ancestor's style reachable through `StyleResolver.ResolvedAncestor` and a whole-subtree edge in `StyleUpdater.Drain` below a named element whose style moved; `or` and one `not` landed with it. ✅ The mixed `(min-width: …) and style(…)` form landed last, as a style group nested in a size group, both halves asking the nearest size container; `or` across the halves landed after it, as a size group beside the style group read as a disjunct, with `or` and `not` in the size half; `style()` on a standard property and parenthesised groups stay refused with a reason. ⚠ The `cq*` units landed — all six of them, read off the style walk rather than off the scope chain, because a container unit needs no `@container` rule and `Recontain` never runs for a document without one. Containment is *free* for a normal-flow block, whose inline size is already `SizingMode.StretchFit`. See § D3 | cascade + layout | 0.1 | 0.6 |
...
| B2 🟡 | ⚠ **Grid landed and this row said 🔴 for months after it did.** `Vixen.Ui.Layout`'s README is the state — ⚠ and this row quoted a set of counts from it that were three generations old, 2 038 / 40 refused / 42 failing, while `TaffyGridConformanceTests` and `Taffy/GridKnownGaps.txt`'s generated `# COUNTS` line already disagreed with them. The counts live there and are named nowhere else — ⚠ including here, because the figures this sentence used to quote in their place have themselves moved since it was written. What is DONE is placement, the bulk of track sizing, baseline alignment, the out-of-flow containing block and `grid-template-areas`. 🟡 rather than 🟢 because **named lines written into a track list** are still owed, which is the one part the README also names. The estimate below is what it cost, not what is left. **CSS Grid** — a separate algorithm; `grid-template-*`, `fr`, `minmax`, `repeat`, `auto-flow`, named lines and areas, placement, `justify/align-items/self`. Judged by B0's **2 040** plus WPT's 510 `check-layout` grid tests. ⚠ B0's corpus does **not** cover `grid-template-areas`: Taffy's own XML harness leaves it `Default::default()` and no fixture sets it, so named areas need their own oracle | **#27** | 3.5 |
| B3 🟡 | **Inline formatting — partially landed.** Line boxes over the existing store: atomic inlines (`inline`, `inline-block`, `inline-flex`), §10.3.9 shrink-to-fit, §9.4.2 line breaking, §10.8.1 baselines including the last-line-box and `overflow` clauses, three of `vertical-align`'s eight values, and **fragmentation**. ⚠ **The boundary used to be one invariant** — every algorithm in the store preserved *one node produces one box*, and a non-replaced `inline` box crossing a line break is fragmented into several. **That invariant has now been relaxed for one arena and three ints** (offset, count and capacity, addressed exactly as `ChildArena` and `TrackArena` are). `FragmentArena` is variable-length *output*, the shape `TrackArena` is on the input side; `FragmentCount == 0` still means "one box, and it is `Position`", so `GetLeft`, the absolute walk and all four of `UiElement`'s rectangle properties were untouched, and a fragmented node's own rectangle is the **union** — which is CSS 2.1 §10.1's containing block for an abspos descendant of an inline box, so the absolute walk needed nothing. The zero-allocation gate holds with a span re-fragmenting every frame. ⚠ **Six of this sentence's items have closed and it had not been rewritten, so read it as a record of what was owed rather than of what is.** Landed since: fragmentation of *nested* spans, to any depth — the blocker was never the nested union, it was one box's fragments being a contiguous slice of a shared scratch that two open boxes cannot both own; fragmentation of a span with an out-of-flow child, **and** §10.6.4's static position for one, which turned out to be a missing `Display.Inline` in the *reader* rather than anything about the recording; anonymous block boxes, which cost no representation at all because §9.2.1.1's box has initial values for every non-inherited property and needs no stored rectangle; the strut and with it the five font-relative `vertical-align` values plus CSS's `<length>`/`<percentage>`, on the finding that a strut is FIVE NUMBERS and only producing them needs a font; and `text-align`. ⚠ **Still owed under B3**: generated boxes; `white-space`'s `pre-line`, which needs CSS Text §4 space collapsing that nothing in `Vixen.Ui` does today — turning it on for `normal` moves every label in the tree. ⚠ **And `break-spaces` was in that sentence for a reason that cannot be true of it**, corrected 2026-09-10: that value *preserves*, exactly as `preserve` does, so collapsing is the third of the property it already agrees with this engine about. CSS Text § 3.1 separates it from `pre-wrap` by two rules and neither is collapsing — a preserved trailing space does not hang, and there is a break opportunity after every one of them *including between two*. Each is a place rather than a subsystem: `LineWrapper.Width`'s trailing trim (`LineWrapper.cs:779`) and UAX #14's LB7/LB18 in `LineBreaker`. `WhiteSpaceBreakSpacesTests` pinned this engine's current answer to both — and ⚠ **`break-spaces` landed on that recipe**: `LineWrapper.Wrap`'s `breakSpaces` flag turns the trim off for the fit and the reported width and adds the opportunity after every space *over* UAX #14's answer rather than inside `LineBreaker`, `UiDocument.BreakSpacesOf` reads the value, `TextLine.Trimmed` carries the spaces into the intrinsic measure, and the test file holds each rule as a pair against the `pre-wrap` control at the same width. Only `pre-line` is owed under `white-space` now. ⚠ Measuring it also turned up a defect rather than a gap — **#1211**: the trim reaches the line's *reported* width only where the wrapper chose the break, so a right-aligned label ending in two spaces draws 8.31 points short of its edge, which is the ragged edge that trim's own remark says it exists to prevent. ⚠ **`pre` is NOT in that list any more and the refusal that put it there was arithmetic that had stopped being true**: `UiDocument.WrapsOf` said honouring `pre` for wrapping alone would be "honouring a third of it", and because this engine collapses nothing and already breaks at every mandatory opportunity, an undeclared element is *already* behaving as `pre-wrap` — so wrapping is the only third `pre` was ever missing. It is honoured, `WhiteSpacePreTests` measures both halves of that premise rather than asserting the conclusion, and the two things in the tree that write it (the editor's console stack trace and its message detail body) stop wrapping mid-frame. Also owed: bidi reordering of inline-level *boxes*, which `Vixen.Ui.Layout` cannot reach (two `ProjectReference`s, neither `Vixen.Ui.Text`) and whose harder half is `PlaceLine`'s Open/Close nesting, since a reversed run crossing a box boundary has to fragment; a text leaf's first line shortened to the space left on the line it lands on; and the INLINE-LEVEL form of `text-overflow: ellipsis` and `line-clamp` — ⚠ **not the properties, which shipped weeks before this row listed them as owed** (`fa9e32b82`, 2026-08-22, and `edc480226`, 2026-09-04), are read at `UiDocument.EllipsisOf`, are called from `UiElement.cs:1116` and are what `ControlTheme.vcss`'s `truncate` writes. What is owed is truncating a LINE BOX shared by several inline-level boxes rather than one element's own text, which is the row `InlineKnownGaps.txt` carries and which names Chromium's `layout/inline/line_truncator.cc` as the reference. ⚠ And the reason recorded for that divergence has itself expired in one clause: `EllipsisOf`'s remark says "Vixen has no shared line box to put them on", citing *one node produces one box* — the invariant B3's own fragmentation work relaxed. The store builds shared line boxes now; what still separates them from the ellipsis is that `UiElement` measures and draws each element's text on its own. ⚠ **Zero fixtures**, confirmed by enumeration — Taffy's `display` attribute takes five values across all eight files and none is inline. Oracle fetched from WPT (`css-flexbox/inline-flex.html`); fragmentation is arithmetic over explicitly sized boxes in `InlineFragmentationTests`. See `InlineKnownGaps.txt`. | **#26** | 2.3 of 3.0 |
...
| B4 🟡 | `display: table` and the four table utilities. ⚠ **Audited five times and not started, and the fifth audit added the one measurement the other four did not take: nothing in this repository asks for it.** Across all 104 committed `.vxml` and `.vcss` files, not one writes `display: table`, `table-layout`, `caption-side`, `border-collapse` or `border-spacing`; the editor's own tabular panels are grid and flex. So this is a parity item rather than a blocked consumer, and the order it lands in is a choice rather than a dependency. ⚠ A `display: table` that is not in `LayoutStyleBuilder`'s eight keywords is dropped by `TryKeyword` — which is why aliasing it onto `Block` to make the keyword resolve would be worse than the silence: a box that reads as a table and lays out as a block. ⚠ **The sixth audit stopped repeating that warning and gated it.** `DisplayKeywordSurfaceTests` pins the accepted keyword set against `Display`'s own members — so a ninth box type cannot arrive unnamed — and denies fourteen CSS values by name, ten of them table-internal, each asserted to leave the DEFAULT. Sabotage-checked both ways: aliasing `table` onto `Block` reddens the denial, adding a `Display.Table` member reddens the coverage. ⚠ And one correction the audits had carried forward: an unread keyword does **not** leave "the display it had". The cascade collapses to one declaration per property before the bridge sees it, so a `display: grid` overridden by a `display: table` is lost the way it would be in a browser and the element falls to the default — the silence is total rather than partial, which is the sharper form of the same argument, since there is no residue of the author's intent to half-honour. | — | 1.0 |
| | | **B total** | **9.4** |
