...
| State | Meaning | Roots |
|---|--:|--:|
| **works** | Vixen emits it, and a consumer acts on every property it sets | **151** |
...
| B1 🟢 | **`display: block` — landed.** Block formatting over the existing store: stacking, the inline-axis fill, CSS 2.1 §8.3.1 margin collapsing in full, auto margins, the intrinsic-width probe, RTL, relative insets, `align-content` over the stack. **768 of the 912 `block`+`blockflex` fixtures pass**; 124 are refused for `scrollbar-width`/`text-align`/`flow-root`/`float`, and all 20 failures are in the shared *absolute* path (`aspect-ratio` re-applied after clamping) rather than in block formatting — auto margins on an abspos box were the other half of that list and landed with CSS Grid §9. ⚠ **Still owed under B1**: `inline-block` — deliberately unmapped, because without an inline formatting context it would take the whole line (B3); the 84 `float` fixtures, which were never gated on `display`; and `sticky`. | **#25** | 0.35 |
