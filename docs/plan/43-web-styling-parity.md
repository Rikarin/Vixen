...
variants wait behind the generated box, carried through six audits and never checked. It is **21 of
...
row stays `partial` on four arrangements the device still declines and counts — a transformed group, a
blended group that also carries a colour matrix or a mask, a blended group's drop-shadow quad, and a
top-level HUD panel in a world renderer, whose capture cannot contain a scene not yet drawn — which its
`value_gap` names.
