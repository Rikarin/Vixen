...
**5. `style()` queries** — ✅ **the unnamed, style-only form landed (#273)**, and the named and mixed
forms are refused with a reason. It is a different shape from the size half: `ContainerQuery`
evaluates a box, a style query evaluates a custom property's computed value, and CSS Conditional 5
...
⚠ **Still refused: the mixed form** (`(min-width: …) and style(…)`). It asks the nearest *size*
container, which `ContainerScopes` holds only as a box and the cascade holds only as a style, so
neither side has both halves. Also refused: `style()` on a standard property, and parenthesised
groups.

**Size: at most 0.1 EM remaining**, from 0.4, and not re-costed. The variants, the marker family, the
units, the coercion and the unnamed and named `style()` forms landed. The mixed form is what is left.
