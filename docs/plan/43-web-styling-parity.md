...
all' is not a container feature" was what every style query in a sheet used to produce. ⚠ **The
named form (`@container card style(…)`) and the mixed form (`(min-width: …) and style(…)`) are
refused on purpose**: both ask an ancestor that may be above the parent, and `StyleUpdater` stops
descending where a style did not move, so a grandparent whose value changed under a parent whose
style did not would never reach the element asking. The unnamed form has exactly inheritance's
invalidation, which `ContainerWiringTests` exercises through a class toggled twice on a live
document. Owed: those two forms, which want the query container's style reachable from the element
and an invalidation edge from it; and `or`/`not`, which neither query grammar here has.

**Size: 0.1 EM remaining**, from 0.4. The variants, the marker family, the units, the coercion and
the unnamed `style()` form landed; the named and mixed `style()` forms are what is left.
...
that also carries a colour matrix or a mask, a blended group's drop-shadow quad, and a top-level HUD
panel in a world renderer, whose capture cannot contain a scene not yet drawn — which its `value_gap`
names. (A transformed group was a fourth until #1379: it was declined because Raven was said to have
no fragment-position input, which was false, and `UiBlend` now reads `SV_Position` for it.)
