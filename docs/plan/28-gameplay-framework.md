...
| G-Q2 | Does the kernel ship a UI layer for these features? | **No, and this is worth being firm about.** `Vixen.Ui` plus the data model is the answer; a shipped inventory window is a shipped art style. Ship them in `Samples/14-Mmo` as copyable reference instead. ✅ **Built** — `Mmo.Ui`: eight VXML components over signal-backed view models, one `vixen.ui.yaml`, and a sixty-line stylesheet because everything a class name can say is a class name. It is the utility system's first consumer and the first VXML outside the editor. ⚠ **Three traps it found, all silent**: a component parameter that is not signal-backed is assigned *after* `Build` and therefore reaches no effect, so the panel draws its default model for ever; a pattern variable does not cross a `@if`, because the condition becomes an effect and the body a separate build; and text content is a `<text>` child element rather than the parent's own text. ⚠ **And one the scanner cannot help with**: a class name assembled at run time is invisible to it, so `border-{rarity}` emits nothing and the fix is a safelist or a switch that writes each name whole |
