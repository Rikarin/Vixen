...
since: 0.2
status: preview
related: [ui/tutorial, editor/inspectors-in-markup, ui/key-value-list, ui/reactive-collections, ui/markup-project-setup, ui/async-loading]
---
...
that cannot be missed. That is how to run the rest of it.

**The sweep is finished, and the last five projects — `Vixen.Editor.Ui`, `Vixen.Editor.Debugger`,
`Vixen.Editor.Profiler`, `Samples/02-HelloUi` and `Samples/14-Mmo/Mmo.Ui` — are zero too.** Read
rather than built, and the reading is short: between them they own fourteen component-flavoured
`.vxml` files and three code-behind partials, all three of which have empty bodies
(`public sealed partial class NetworkView;` and its two siblings), and the only public settable
properties declared anywhere in those files are `Action<T>` callbacks, which the rule exempts.

⚠ **That zero used to be structural rather than earned, and both reasons have since been fixed.** The
sweep numbers above were taken while the rule could not see most parameters and reported some it
recommends; they are kept here because the projects and files they name are still the ones to sweep,
not because a fresh run would print the same thing.

⚠ **A parameter declared in a `@code` block is now reported — against the `.vxml`.** The emitter
writes `// <auto-generated />` as the first line of the file it copies the block into, so under
`ConfigureGeneratedCodeAnalysis(None)` such a declaration was neither analyzed nor reported, and that
is exactly where this repository's markup components put their parameters: the `Model` in the block
above is one, and it is the property the whole issue was filed about. The analyzer now asks for
`Analyze` and withholds `ReportDiagnostics`, and re-creates the diagnostic at the location the
emitter's `#line` span maps to. So a hit lands on the characters the author wrote in the `.vxml`,
and generated code that *no* directive maps stays silent — which was the whole objection to analyzing
it.

⚠ **And the reactive test asks about the accessors, not only about the type.** "Back it with a
`Signal<T>`" describes exactly the `Model` above, and a property of that shape used to be reported,
because the test looked at the property's type — `ShellModel` — and not at what the getter touches.
It does track: an effect reading it reads `model.Value` and subscribes. A getter that reads anything
from `Vixen.Ui.Reactive` now counts as reactive however plain the property's own type is, so the rule
no longer complains about the shape it recommends. A getter with a body that reads nothing reactive
is still reported, which is what stops this from being an opt-out.

So `VXS0320` answers "can an effect that reads this property subscribe to anything", which is the
question the message was always about. It stays a suggestion: a plain parameter is sometimes right,
and promoting it is still a decision to take after a real sweep.
