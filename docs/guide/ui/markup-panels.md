...
⚠ **Two structural reasons make that zero less reassuring than it sounds, and together they say the
promotion should not happen at all.**

⚠ **A parameter declared in a `@code` block is invisible to the rule.** The emitter writes
`// <auto-generated />` as the first line of the file it copies the block into, and the analyzer sets
`ConfigureGeneratedCodeAnalysis(None)`, so such a declaration is neither analyzed nor reported —
whether the class is wholly generated or is a partial with a hand-written half. That is where this
repository's markup components put their parameters: the `Model` in the block above is one of them,
and it is the property the whole issue was filed about.

⚠ **And the rule fires on the shape its own message asks for.** "Back it with a `Signal<T>`" describes
exactly the `Model` above, and a property of that shape *is* reported, because the reactive test looks
at the property's type and not at what the accessors touch. Such a property does track — an effect
reading it reads `model.Value` and subscribes — so under `TreatWarningsAsErrors` a promotion to
warning would make the recommended pattern a build error. Both facts are pinned by
`ComponentParameterTests`, not left as prose.

So `VXS0320` answers "is this property obviously reactive from its type", which is a useful
suggestion at a declaration and is not a gate.
