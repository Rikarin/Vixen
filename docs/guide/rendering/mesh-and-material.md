...
| `MaterialFeatureStage` | `Vixen.Rendering.Materials` | Whether a feature writes the surface *coordinate* or a channel. ⚠ A `Coordinate` feature has to be **first** in a material's `Features` list, and `MaterialCompiler` refuses one that is not — the chain runs in list order, so a displacement in slot 3 would leave every feature ahead of it sampling at the coordinate it is about to replace. Half a surface displaced, drawn without an error. ⚠ **Nothing declares `Coordinate` yet**: the rule is deliberately dormant, written before the feature that needs it ([#1065](https://github.com/Rikarin/Vixen/issues/1065)), and `MaterialFeatureOrderTests` reads `Raven/Library` so the day a shader body assigns `d.uv` the C# either declares it or the suite is red |
