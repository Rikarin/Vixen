...
///         called <see cref="UvUnwrap.All(EditMesh, UvSettings, PackSettings, out IReadOnlyList{Vector2})" />.</b> <see cref="UvPackPropertyTests" /> states its half
///         over <see cref="UvUnwrap.Pack(IReadOnlyList{UvIsland}, PackSettings)" />, which is the <i>third</i> stage and is handed islands a
...
public class UvUnwrapPipelinePropertyTests {
    ...
    ///         <see cref="UvUnwrap.All(EditMesh, UvSettings, PackSettings, out IReadOnlyList{Vector2})" /> packed, not a re-derivation that might not be.
    ///     </para>
    ///     <para>
