...
public class RemeshReportTests {
    ...
    public void Every_stage_reports_what_it_handled(string name) {
        ...
        // ⚠ Stage seven is R5's and is recorded at zero so that the report's shape does not change when
        // attribute transfer lands. Asserting the zero is what makes that a promise rather than a note.
        Assert.Equal(0, stages[RemeshStage.Transfer].Elements);
        Assert.Equal(TimeSpan.Zero, stages[RemeshStage.Transfer].Elapsed);
