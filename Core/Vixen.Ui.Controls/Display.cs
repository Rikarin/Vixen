...
public sealed partial class KeyboardShortcut : Control {
    ...
    /// <remarks>
    ///     ⚠ <b>A forwarder to <see cref="ShortcutFormat.Formatter" />, which is where it lives now.</b>
    ///     Neither the formatter nor the key-name table was ever view state, and a keymap in
    ///     <c>Vixen.Ui</c> could not reach them while they were statics on a <c>Control</c>. Kept here
    ///     rather than deleted because a label whose whole job is drawing a chord is where an
    ///     application looks first, and there is exactly one setting behind both names.
    /// </remarks>
    ...
    /// <remarks>A forwarder to <see cref="ShortcutFormat.Describe" />, which carries the reasoning.</remarks>
