        ...
        foreach (var member in type.GetMembers()) {
            ...
            switch (member) {
                ...
                case IPropertySymbol { IsIndexer: false, GetMethod: not null } property
                    ...
                    // A property with no setter at all has nowhere to put a value, so it is derived
                    // from something else that is already being written. `Vector3.Length` is the
                    // example, and writing it would be writing the same number twice.
            ...
            members.Add(Describe(member.Name, memberType, settable, computed, OrderOf(member), sequence++));
        }
    }

    static MemberModel Describe(string name, ITypeSymbol type, bool settable, bool computed, int order, int sequence) {
        var qualified = type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
        var keyword = Keyword(type) ?? qualified;

        if (Primitives.TryGetValue(keyword, out var primitive)) {
            return new(name, keyword, MemberShape.Primitive, primitive, string.Empty, string.Empty, settable, computed, order, sequence);
        }
        ...
        if (type.TypeKind == TypeKind.Enum && type is INamedTypeSymbol { EnumUnderlyingType: { } underlying }) {
            ...
            return new(name, qualified, MemberShape.Enum, Primitives[underlyingKeyword], underlyingKeyword, string.Empty, settable, computed, order, sequence);
        }
        ...
        if (type is IArrayTypeSymbol { Rank: 1 } array) {
            ...
            return new(name, qualified, shape, string.Empty, element, string.Empty, settable, computed, order, sequence);
        }
        ...
        if (type is INamedTypeSymbol { IsGenericType: true } generic) {
            ...
            switch (definition) {
                case "global::System.Collections.Generic.List<T>":
                    return new(name, qualified, MemberShape.List, string.Empty, first, string.Empty, settable, computed, order, sequence);

                case "global::System.Collections.Generic.Dictionary<TKey, TValue>":
                    return new(name, qualified, MemberShape.Dictionary, string.Empty, first, Argument(generic, 1), settable, computed, order, sequence);

                case "global::System.Nullable<T>":
                    return new(name, qualified, MemberShape.Nullable, string.Empty, first, string.Empty, settable, computed, order, sequence);
                ...
                case "global::Vixen.Core.Serialization.ContentReference<T>":
                    return new(name, qualified, MemberShape.Reference, string.Empty, first, string.Empty, settable, computed, order, sequence);
            }
        }
        ...
        return new(name, qualified, otherShape, string.Empty, string.Empty, string.Empty, settable, computed, order, sequence);
    }
