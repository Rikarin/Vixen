...
static class BindingsEmitter {
    ...
    /// <param name="shaderName">The shader, which names every type emitted here.</param>
    /// <param name="reflection">What Raven said about it.</param>
    /// <param name="source">The reflection file, named in the header so a reader can find it.</param>
    /// <param name="isInternal">
    ///     Whether the emitted types are internal rather than public. ⚠ Set by an assembly whose
    ///     shader names could collide with a referenced assembly's — see
    ///     <c>ShaderBindingsGenerator.InternalProperty</c>, which argues it.
    /// </param>
    public static string Emit(string shaderName, ShaderReflection reflection, string source, bool isInternal = false) {
        ...
        EmitKeys(writer, shaderName, reflection, blocks, visibility);

        foreach (var block in blocks) {
            writer.AppendLine();
            EmitConstants(writer, shaderName, reflection, block, blocks.Length > 1, visibility);
        }
    ...
    static void EmitKeys(StringBuilder writer, string shaderName, ShaderReflection reflection, UniformBlockInfo[] blocks, string visibility) {
        writer.AppendLine("/// <summary>");
        writer.AppendLine($"///     Typed keys for shader <c>{Escape(shaderName)}</c>.");
        writer.AppendLine("/// </summary>");
        writer.AppendLine($"{visibility} static class {shaderName}Keys {{");
    ...
    static void EmitConstants(
        ...
        foreach (var array in arrays) {
            EmitElement(writer, shaderName, array, owned, visibility);
            writer.AppendLine();
        }
        ...
        writer.AppendLine("/// </remarks>");
        writer.AppendLine($"{visibility} struct {structName} {{");
    ...
    ///     </para>
    /// </remarks>
    static void EmitElement(StringBuilder writer, string shaderName, Member array, Parameter[] owned, string visibility) {
        ...
        writer.AppendLine("/// </summary>");
        writer.AppendLine($"{visibility} struct {name} {{");
