...
///     </para>
///     <para>
///         ⚠ <b>Nothing registers this system in a game's loop, so no shipping frame reaches any of
///         it.</b> <c>AnimationSystems.AddAnimation</c> is called only by <c>GizmoTests</c> and no
///         animation system carries <c>[GameSystem]</c>, so the generated registry adds none of them —
///         which also means <see cref="AnimationSystem" /> is not running and an
///         <see cref="AnimatorComponent" /> in a shipping game is never evaluated at all. That is
///         <a href="https://github.com/Rikarin/Vixen/issues/1221">#1221</a> and it is a link below
///         this one; and even once it is closed, <see cref="Renderer" /> and <see cref="Virtualized" />
///         have to be handed over by something that can see both this assembly and the renderer's —
///         <c>Vixen.Engine.Renderer</c> has no reference to <c>Vixen.Animation</c>, so
///         <c>WorldRenderer.Register</c> cannot be that place and the application host has to be.
///     </para>
///     <para>
