...
public sealed class AiSystem : SystemBase, IDeclaredAccess {
    ...
    void Tick(World world, GameTime time, Entity entity, ref AiAgent agent) {
        ...
        if (agent.Status != ActionStatus.Running) {
            // Restarted rather than left finished. With no planner above it there is nothing to
            // decide what a finished action means, and an agent that stopped for ever would look
            // like a bug in the substrate rather than the absence of a planner. A behaviour tree
            // takes this over in P1 and the restart goes with it.
            agent.Started = false;
        }
