...
| `position: sticky` | The third contributor to that same sum, resolved by `StickyReader`. ⚠ It is here rather than in `Vixen.Ui.Layout` for a reason that is a fact rather than a preference: a sticky offset is a function of a *scroll* offset, and that store has none — `ScrollView` scrolls by writing `OffsetY`, which never reaches the layout tree. A box is floored at its scrollport's edge and then clamped inside its own containing block; the second clamp is the whole difference between `sticky` and `fixed`, which doc 09 refuses because a game overlay has no viewport. ⚠ The *offset* is what cannot live in the layout store; the *box* can and now does — `PositionType.Sticky` is a containing block that ignores its own inset, so an absolutely positioned descendant anchors to the sticky box the way CSS Position 3 § 2 says, without the `contain: layout` `AdvancedTheme.vcss` used to need beside it. |
...
⚠ **What runs a command is here; what a keystroke means is one assembly up.** There is no chord table
in `Vixen.Ui` and no `.vxml` spelling of a shortcut — `MenuItem.ShowShortcut(key, modifiers)` *draws*
one and registers nothing, which is why `Samples/02-HelloUi/Shell.vxml` says in as many words that a
menu claiming ⌘S would be lying. `KeyChord` is this assembly's; the table over it is
`Vixen.Ui.Controls`'s, where `MenuItem` itself lives: `KeyMap` composes defaults, a preset and the
user's overrides, and `CommandDispatcher` attaches to any `UiDocument`, turns a `KeyEvent` into a
...
*afterwards*, `DrawList.Version`, which is the first part of `UiGeometryBuilder.TryBuild`'s key and
lets a still window keep its geometry rather than flatten and tessellate again, and does not skip
...
change reaches when the extent does not move). And `UiRenderer.GeometryUploads` climbs once while
`GeometryUploadsSkipped` takes the rest — the copy to the device is keyed on
`UiGeometry.Generation`, so a frame answered with the geometry the builder already held draws from
the region the device already has, and allocates and writes nothing. What is still paid every frame
besides the rebuild is the command recording itself, because the swapchain hands back a different
image each frame and something has to draw into it. An earlier version of this paragraph said the
GPU recording was skipped; it never was, and the upload it did not mention was paid every frame.
