...
/// <remarks>
///     <para>
///         Emits <b>loose triangles</b> — every three points is one — rather than vertices and
...
///         permutation and cost a second array to say so. The caller turns the positions into
///         whatever vertex its shader reads, which is also where the colour comes from.
///     </para>
///     <para>
///         ⚠ <b>Nothing here is antialiased, and that is a real limitation rather than an oversight.</b>
///         A rounded box gets a perfect edge because the shader evaluates a distance field, and a
///         glyph gets one for the same reason; a tessellated path arrives at the rasteriser as
///         triangles and gets whatever the pass gives it. Multisampling the pass fixes it and is the
///         compositor's decision; a feathered fringe along the outline fixes it more cheaply and is
///         owed. What is not owed is silence about it — a jagged icon edge is visible and bounded,
///         which is a different thing from a shape that is wrong.
///     </para>
/// </remarks>
public static class PathTessellator {
    ...
    static void Trapezoid(List<Vector2> triangles, Crossing left, Crossing right, float top, float bottom) {
        var lt = left.Top;
        var rt = right.Top;
        var lb = left.Bottom;
        var rb = right.Bottom;
    ...
    public static void Stroke(
        ...
        foreach (var contour in contours) {
            ...
            for (var i = 0; i < last; i++) {
                var from = points[contour.First + i];
                var to = points[contour.First + ((i + 1) % contour.Count)];
                Segment(triangles, from, to, half);
            }
            ...
            for (var i = first; i < last; i++) {
                ...
                Join(triangles, previous, vertex, next, half, join, tolerance, miterLimit);
            }
            ...
            Cap(triangles, start, Direction(second, start), half, cap, tolerance);
            Cap(triangles, end, Direction(penultimate, end), half, cap, tolerance);
        }
    }

    /// <summary>One segment's quad.</summary>
    static void Segment(List<Vector2> triangles, Vector2 from, Vector2 to, float half) {
    ...
    static void Join(
        ...
        switch (join) {
            ...
            case LineJoin.Round: {
                ...
                Arc(triangles, vertex, from, sweep, MathF.Abs(side), tolerance);
                break;
            }
    ...
    static void Cap(List<Vector2> triangles, Vector2 end, Vector2 direction, float half, LineCap cap, float tolerance) {
        if (cap == LineCap.Butt || direction == Vector2.Zero) {
            return;
        }
        ...
        Arc(triangles, end, end - normal, MathF.PI, half, tolerance);
    }
    ...
    /// </summary>
    static void Arc(List<Vector2> triangles, Vector2 centre, Vector2 from, float sweep, float radius, float tolerance) {
    ...
    static void Triangle(List<Vector2> triangles, Vector2 a, Vector2 b, Vector2 c) {
        triangles.Add(a);
        triangles.Add(b);
        triangles.Add(c);
    }
